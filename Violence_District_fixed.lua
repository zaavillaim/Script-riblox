...
-- =====================================================
-- RE-EXECUTE CLEANUP: Destroy UI dari run sebelumnya
-- =====================================================
do
    -- 1. Destroy old main window (ModernV2 ScreenGui) yang nyebabkan input keblok
    pcall(function()
        if getgenv().KYS_MainWindow then
            local w = getgenv().KYS_MainWindow
            if type(w) == "table" then
                if type(w.Destroy) == "function" then pcall(function() w:Destroy() end)
                elseif type(w.destroy) == "function" then pcall(function() w:destroy() end) end
            end
            getgenv().KYS_MainWindow = nil
        end
    end)

    -- 2. Cari dan destroy semua ScreenGui sisa dari run sebelumnya
    pcall(function()
        local function cleanupGuis(parent)
            if not parent then return end
            local names = {
                "KYS_MobileUI", "KYS_AimBtn", "SpearAimbotUI", "KYS_Crosshair",
                "KYS_RadarGui", "KYS_CrosshairGui"
            }
        ...
        cleanupGuis(lp:FindFirstChild("PlayerGui"))
        pcall(function() cleanupGuis(game:GetService("CoreGui")) end)
        pcall(function() if gethui then cleanupGuis(gethui()) end end)
    end)

    -- 3. Reset VD.Destroyed supaya loop dan heartbeat jalan lagi
    pcall(function()
        if getgenv().VD then getgenv().VD.Destroyed = false end
    end)

    -- 4. Reset KYS_MobileGui supaya CreateMobileUI buat instance baru
    getgenv().KYS_MobileGui = { AimBtn = nil, FOVFrame = nil, FOVStroke = nil }

    print("[KysHub] Cleanup selesai, inisialisasi ulang...")
end
-- =====================================================
...
if ModernV2 then
    ...
    -- Simpan referensi Window untuk cleanup saat re-execute
    getgenv().KYS_MainWindow = Window

    -- Buka window supaya langsung muncul saat pertama kali execute
    task.spawn(function()
        task.wait(0.1)
        pcall(function()
            if type(Window.Open)   == "function" then Window:Open()
            elseif type(Window.Show)   == "function" then Window:Show()
            elseif type(Window.Toggle) == "function" then Window:Toggle(true) end
        end)
    end)
...
local VisualFeatureTabs, MainFeatureTabs, MainKillerFeatureTabs, AimFeatureTabs, MappingFeatureTabs, PlayerFeatureTabs, PlayerMiscFeatureTabs, PlayerPerfFeatureTabs
...
if Window then
    ...
    PlayerPerfFeatureTabs = addCenterFeatureTabbox(Tabs.Player, "Performance", {
        { Key = "PingBoost", Name = "Ping Boost", Icon = "solar:wifi-router-minimalistic-bold" },
        { Key = "AntiLag",   Name = "Anti Lag",   Icon = "solar:chart-2-bold" },
    })
...
-- =====================================================
do
    local pingBoostSection = PlayerPerfFeatureTabs.PingBoost:AddSection({
        Position = "Center",
        Name     = "Ping Boost",
    ...
    pingBoostSection:AddToggle({
        Name     = "Ping Boost",
        Flag     = "PERF_PingBoost",
        Default  = false,
        Callback = function(val)
            VD.PERF_PingBoost = val
            KYS_ApplyPingBoost(val)
        end,
    })

    pingBoostSection:AddParagraph({
        Name    = "Info",
        Title   = "Tentang Ping Boost",
        Content = "Mengoptimasi koneksi client: menonaktifkan replikasi objek jauh & membuang antrian preload. Cocok saat ping tinggi / koneksi lemot.",
    })
end
...
-- =====================================================
do
    local antiLagSection = PlayerPerfFeatureTabs.AntiLag:AddSection({
        Position = "Center",
    ...
    antiLagSection:AddToggle({
        ...
        Callback = function(val)
            VD.PERF_AntiLag = val
            KYS_ApplyAntiLag(val)
        end,
    })

    antiLagSection:AddSlider({
        Name      = "Level Optimisasi",
        Flag      = "PERF_AntiLagLevel",
        Min       = 1,
        Max       = 3,
        Default   = 3,
        Increment = 1,
        Suffix    = "",
        Callback  = function(val)
            VD.PERF_AntiLagLevel = val
            if VD.PERF_AntiLag then
                KYS_ApplyAntiLag(false)
                task.wait(0.1)
                KYS_ApplyAntiLag(true)
            end
        end,
    })

    antiLagSection:AddParagraph({
        Name    = "Level Info",
        Title   = "Level Penjelasan",
        Content = "Lv 1 = Matikan shadow\nLv 2 = + Matikan partikel/asap\nLv 3 = + Kurangi terrain & fog (max FPS)",
    })
end
...
-- =====================================================
-- PING BOOST IMPLEMENTATION
-- =====================================================
local KYS_PingBoostState = {
    FocusPart = nil,
    Active    = false,
}

function KYS_ApplyPingBoost(enabled)
    KYS_PingBoostState.Active = enabled
    VD.PERF_PingBoost = enabled

    if enabled then
        -- Pasang ReplicationFocus di dekat karakter sendiri supaya
        -- server hanya replikasi objek dekat player → kurangi bandwidth
        pcall(function()
            local char  = Players.LocalPlayer.Character
            local hrp   = char and char:FindFirstChild("HumanoidRootPart")

            local focus = workspace:FindFirstChild("KYS_PingFocus")
            if not focus then
                focus            = Instance.new("Part")
                focus.Name       = "KYS_PingFocus"
                focus.Anchored   = true
            ...
            if hrp then
                focus.CFrame = hrp.CFrame
            end

            Players.LocalPlayer.ReplicationFocus = focus
            KYS_PingBoostState.FocusPart = focus
        end)

        -- Update posisi focus setiap frame supaya selalu ikut karakter
        if not KYS_PingBoostState.MoveConn then
            KYS_PingBoostState.MoveConn = RunService.Heartbeat:Connect(function()
                if not VD.PERF_PingBoost then
                    if KYS_PingBoostState.MoveConn then
                        KYS_PingBoostState.MoveConn:Disconnect()
                        KYS_PingBoostState.MoveConn = nil
                    end
                    return
                end
                pcall(function()
                    local char = Players.LocalPlayer.Character
                    local hrp  = char and char:FindFirstChild("HumanoidRootPart")
                    local f    = KYS_PingBoostState.FocusPart
                    if hrp and f and f.Parent then
                        f.CFrame = hrp.CFrame
                    end
                end)
            end)
        ...
        -- Buang antrian preload ContentProvider (hemat bandwidth)
        pcall(function()
            game:GetService("ContentProvider"):PreloadAsync({})
        end)

        pcall(VD_Notify, "Ping Boost", "Aktif! Replikasi dioptimasi.", 3)
    else
        -- Kembalikan ReplicationFocus ke default (nil = server managed)
        pcall(function()
            Players.LocalPlayer.ReplicationFocus = nil
        end)
        pcall(function()
            local f = workspace:FindFirstChild("KYS_PingFocus")
            if f then f:Destroy() end
            KYS_PingBoostState.FocusPart = nil
        end)
        if KYS_PingBoostState.MoveConn then
            KYS_PingBoostState.MoveConn:Disconnect()
            KYS_PingBoostState.MoveConn = nil
        end
        pcall(VD_Notify, "Ping Boost", "Dinonaktifkan.", 2)
    end
end
...
-- =====================================================
local KYS_AntiLagState = {
    Active              = false,
    OrigShadows         = nil,
    OrigShadowSoftness  = nil,
    OrigFogEnd          = nil,
    OrigFogStart        = nil,
    OrigDecoration      = nil,
    ParticleAddConn     = nil,
...
local function KYS_SetParticles(enabled)
    -- Iterasi semua descendants workspace
    pcall(function()
    ...
    -- Iterasi Lighting descendants juga
    pcall(function()
...
function KYS_ApplyAntiLag(enabled)
    VD.PERF_AntiLag = enabled

    if enabled then
        KYS_AntiLagState.Active = true
        local level = tonumber(VD.PERF_AntiLagLevel) or 3

        -- Simpan nilai asli sebelum diubah
        pcall(function()
            KYS_AntiLagState.OrigShadows        = Lighting.GlobalShadows
            KYS_AntiLagState.OrigShadowSoftness = Lighting.ShadowSoftness
            KYS_AntiLagState.OrigFogEnd         = Lighting.FogEnd
            KYS_AntiLagState.OrigFogStart       = Lighting.FogStart
        end)
        pcall(function()
            local terrain = workspace:FindFirstChildOfClass("Terrain")
            if terrain then
                KYS_AntiLagState.OrigDecoration = terrain.Decoration
            end
        end)

        -- Level 1: Matikan shadow (penghematan GPU besar)
        if level >= 1 then
            pcall(function()
                Lighting.GlobalShadows = false
        ...
        -- Level 2: Matikan semua partikel, asap, api
        if level >= 2 then
            KYS_SetParticles(false)

            -- Pantau objek baru yang masuk ke workspace dan nonaktifkan partikelnya
            if not KYS_AntiLagState.ParticleAddConn then
                KYS_AntiLagState.ParticleAddConn = workspace.DescendantAdded:Connect(function(v)
                    if not VD.PERF_AntiLag then return end
                    if v:IsA("ParticleEmitter") or v:IsA("Smoke")
                    or v:IsA("Fire")            or v:IsA("Sparkles") then
        ...
        -- Level 3: Kurangi terrain decoration & hapus fog (clarity + FPS)
        if level >= 3 then
            pcall(function()
                local terrain = workspace:FindFirstChildOfClass("Terrain")
        ...
        pcall(VD_Notify, "Anti Lag", "Aktif! Level " .. tostring(level) .. " diaplikasikan.", 3)
    else
        KYS_AntiLagState.Active = false

        -- Putus koneksi partikel auto-off
        if KYS_AntiLagState.ParticleAddConn then
            pcall(function() KYS_AntiLagState.ParticleAddConn:Disconnect() end)
            KYS_AntiLagState.ParticleAddConn = nil
        end

        -- Kembalikan shadow
        pcall(function()
            if KYS_AntiLagState.OrigShadows ~= nil then
                Lighting.GlobalShadows = KYS_AntiLagState.OrigShadows
            end
            if KYS_AntiLagState.OrigShadowSoftness ~= nil then
                Lighting.ShadowSoftness = KYS_AntiLagState.OrigShadowSoftness
            end
        end)

        -- Kembalikan fog
        pcall(function()
            if KYS_AntiLagState.OrigFogEnd ~= nil then
                Lighting.FogEnd = KYS_AntiLagState.OrigFogEnd
            end
            if KYS_AntiLagState.OrigFogStart ~= nil then
                Lighting.FogStart = KYS_AntiLagState.OrigFogStart
            end
        end)

        -- Kembalikan terrain decoration
        pcall(function()
            local terrain = workspace:FindFirstChildOfClass("Terrain")
            if terrain and KYS_AntiLagState.OrigDecoration ~= nil then
                terrain.Decoration = KYS_AntiLagState.OrigDecoration
            end
        end)

        -- Nyalakan kembali semua partikel
        KYS_SetParticles(true)

        pcall(VD_Notify, "Anti Lag", "Dinonaktifkan, pengaturan dikembalikan.", 2)
    end
end
...
function CreateMobileUI()
    ...
    -- Destroy ScreenGui lama sebelum buat yang baru (cegah duplikat)
    pcall(function()
        local old1 = pg:FindFirstChild("KYS_MobileUI")
        if old1 then old1:Destroy() end
        local old2 = pg:FindFirstChild("KYS_AimBtn")
...
local function KillPremium()
    -- Guard: jangan hook ulang kalau sudah pernah dipasang
    if getgenv().KYS_KillPremiumHooked then return end
    getgenv().KYS_KillPremiumHooked = true
