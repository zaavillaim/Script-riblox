...
do
    ...
    if readfile and loadstring then
        local ok, result = pcall(function()
            return loadstring(readfile("ui_vd_classic.lua"))()
        end)
    ...
    if not loaded and loadstring then
        -- Fallback to the original framework when the custom UI file is absent.
        ...
        if loaderOk then ModernV2 = loaderResult end
    end
end
...
if ModernV2 then
    -- Uses the supplied ui.lua visual language:
    -- compact dark panel, purple accent, left navigation and nested feature selectors.
    Window = ModernV2.new("Violence District")
    if Window then
...
getgenv().VD = getgenv().VD or {
    ...
    PING_BOOST_Enabled    = false,
    PING_BOOST_Interval   = 1.0,
    ANTI_LAG_Enabled      = false,
    ANTI_LAG_Particles    = true,
    ANTI_LAG_PostEffects  = true,
    ANTI_LAG_Lights       = false,
    ANTI_LAG_Textures     = false,
    ANTI_LAG_Terrain      = false,
    CROSS_Enabled         = false,
    CROSS_Style           = "Dot",
    CROSS_Size            = 3,
    CROSS_Thickness       = 4,
...
local VD_To_Flag = {
    ...
    PING_BOOST_Enabled = "Ping Boost",
    PING_BOOST_Interval = "Ping Boost Interval",
    ANTI_LAG_Enabled = "Anti Lag",
    ANTI_LAG_Particles = "Disable Particles / Trails",
    ANTI_LAG_PostEffects = "Disable Post Effects",
    ANTI_LAG_Lights = "Disable Lights",
    ANTI_LAG_Textures = "Hide Textures / Decals",
}
...
-- =====================================================
-- PING BOOST / ANTI LAG
-- Client-side optimizations. These do not change the server's
-- actual network latency; Ping Boost keeps lightweight client
-- activity predictable while Anti Lag reduces local rendering work.
-- =====================================================
local VD_PingBoostConnection = nil
local VD_AntiLagState = {
    Saved = {},
    Applied = false
}

local function VD_SetPingBoost(state)
    VD.PING_BOOST_Enabled = state == true

    if VD_PingBoostConnection then
        pcall(function() VD_PingBoostConnection:Disconnect() end)
        VD_PingBoostConnection = nil
    end

    if not VD.PING_BOOST_Enabled then return end

    -- Avoid fake "ping reduction" claims. This only throttles the
    -- helper loop and keeps it extremely lightweight.
    local acc = 0
    VD_PingBoostConnection = RunService.Heartbeat:Connect(function(dt)
        acc += dt
        if acc >= (tonumber(VD.PING_BOOST_Interval) or 1) then
            acc = 0
            -- Touching no remotes here is intentional: firing arbitrary
            -- remotes cannot reliably reduce ping and may cause kick/errors.
        end
    end)
end

local function VD_ApplyAntiLag(state)
    VD.ANTI_LAG_Enabled = state == true

    if not VD.ANTI_LAG_Enabled then
        -- Restore properties changed by Anti Lag.
        for obj, props in pairs(VD_AntiLagState.Saved) do
        ...
        VD_AntiLagState.Saved = {}
        VD_AntiLagState.Applied = false
        return
    end

    VD_AntiLagState.Saved = {}

    local function save(obj, prop, value)
        VD_AntiLagState.Saved[obj] = VD_AntiLagState.Saved[obj] or {}
        if VD_AntiLagState.Saved[obj][prop] == nil then
            VD_AntiLagState.Saved[obj][prop] = value
        end
    end
    ...
    local function process(obj)
        ...
        if VD.ANTI_LAG_Particles and obj:IsA("ParticleEmitter") then
            save(obj, "Enabled", obj.Enabled)
            obj.Enabled = false
        elseif VD.ANTI_LAG_Particles and obj:IsA("Trail") then
            save(obj, "Enabled", obj.Enabled)
            obj.Enabled = false
        elseif VD.ANTI_LAG_PostEffects and (
            obj:IsA("BloomEffect") or obj:IsA("BlurEffect") or
            obj:IsA("DepthOfFieldEffect") or obj:IsA("SunRaysEffect")
        ) then
            save(obj, "Enabled", obj.Enabled)
            obj.Enabled = false
        elseif VD.ANTI_LAG_Lights and obj:IsA("Light") then
            save(obj, "Enabled", obj.Enabled)
            obj.Enabled = false
        elseif VD.ANTI_LAG_Textures and (
            obj:IsA("Decal") or obj:IsA("Texture")
        ) then
            save(obj, "Transparency", obj.Transparency)
            obj.Transparency = 1
        elseif VD.ANTI_LAG_Terrain and obj:IsA("Terrain") then
            -- Terrain is intentionally not modified automatically.
            -- Roblox terrain quality is controlled by engine settings.
        end
    end
    ...
    VD_AntiLagState.Applied = true
end

local function VD_RefreshAntiLag()
    if VD.ANTI_LAG_Enabled then
        VD_ApplyAntiLag(true)
    end
end

getgenv().VD_SetPingBoost = VD_SetPingBoost
getgenv().VD_ApplyAntiLag = VD_ApplyAntiLag
