...
-- =====================================================
-- KYS UI: ui.lua-style standalone UI
-- =====================================================
local KYS_UI_SOURCE = [==[
local a = {}
...
do
    local ok, lib = pcall(function()
        local fn, err = loadstring(KYS_UI_SOURCE)
    ...
    if ok then
        ...
        warn("[KysHub] ui.lua bootstrap failed:", lib)
    end
end
...
local Window = nil
if KYS_UI_Lib then
    Window = KYS_UI_Lib.new("Violence District")
end

if Window then
    local function patchTab(obj)
        if not obj or obj.__KYS_Patched then return obj end
        obj.__KYS_Patched = true

        obj.AddDivider = function(self, cfg)
            return self:section((cfg and (cfg.Text or cfg.Name)) or "")
        end
        obj.AddLabel = function(self, value)
            return self:label(value)
        end
        obj.AddParagraph = function(self, title, content)
            return self:label(tostring(title or "") .. "\n" .. tostring(content or ""))
        end
        obj.AddTextInput = function(self, cfg)
            cfg = cfg or {}
            return self:input(cfg.Name or "Input", cfg.Placeholder or "", cfg.Callback)
        end
        obj.AddSlider = function(self, cfg)
            cfg = cfg or {}
            return self:slider(
                cfg.Name or "Slider",
                tonumber(cfg.Min) or 0,
                tonumber(cfg.Max) or 100,
                tonumber(cfg.Default) or tonumber(cfg.Min) or 0,
                cfg.Callback
            )
        end
        obj.AddToggle = function(self, cfg)
            cfg = cfg or {}
            return self:toggle(cfg.Name or "Toggle", cfg.Default == true, cfg.Callback)
        end
        obj.AddButton = function(self, name, callback)
            if type(name) == "table" then
                callback = name.Callback
                name = name.Name
            end
            return self:button(tostring(name or "Button"), callback)
        end
        obj.AddSection = function(self, cfg)
            if cfg and cfg.Name then self:AddDivider({Text = cfg.Name}) end
            return self
        end
        return obj
    end
    ...
    Window.tab = function(self, name)
        return patchTab(originalTab(self, name))
    end

    Window.AddTab = function(self, cfg)
        return self:tab((cfg and cfg.Name) or "Tab")
    end

    Window.AddCenterTabbox = function(self, _)
        local owner = self
        return {
            AddTab = function(_, cfg)
                return owner:tab((cfg and cfg.Name) or "Tab")
            end
        }
    end
...
local function KYS_SerializeConfigValue(value)
    if typeof(value) == "Color3" then
        return {__KYS_Color3 = true, R = value.R, G = value.G, B = value.B}
    elseif type(value) == "table" then
        local out = {}
        for k, v in pairs(value) do out[k] = KYS_SerializeConfigValue(v) end
        return out
    end
    return value
end

local function KYS_DeserializeConfigValue(value)
    if type(value) == "table" then
        if value.__KYS_Color3 then
            return Color3.new(
        ...
        local out = {}
        for k, v in pairs(value) do out[k] = KYS_DeserializeConfigValue(v) end
        return out
    end
    return value
end
...
function KYS_SaveConfig(name)
    ...
    name = tostring(name):gsub("[^%w_%-%s]", ""):sub(1, 40)
    if name == "" then name = "Default" end

    local path = ConfigFolderName .. "/" .. name .. ".json"
    local payload = {
        VD = VD,
        ESP = getgenv().KYS_VD_VisualESP_State or nil,
    }
    local ok, encoded = pcall(function()
        return HttpService:JSONEncode(KYS_SerializeConfigValue(payload))
    end)
    if ok and encoded and writefile then
        pcall(writefile, path, encoded)
        getgenv().CurrentConfigName = name
        print("[VD Config] Saved:", name)
        return true
    end
    warn("[VD Config] Save failed:", name)
    return false
end
...
function KYS_LoadConfig(name)
    ...
    pcall(function()
        if readfile and isfile and isfile(path) then
            ...
            data = KYS_DeserializeConfigValue(data)

            -- New format stores both main settings and ESP settings.
            -- Keep backwards compatibility with older flat VD-only configs.
            local vdData = (type(data) == "table" and type(data.VD) == "table") and data.VD or data
            for key, value in pairs(vdData or {}) do
                VD[key] = value
            ...
            if type(data) == "table" and type(data.ESP) == "table"
            and type(getgenv().KYS_VD_VisualESP_State) == "table" then
                local espState = getgenv().KYS_VD_VisualESP_State
                for key, value in pairs(data.ESP) do
                    espState[key] = value
                end

                local espFlags = {
                    SurvivorColor = "KYS Survivor Color",
                    KillerColor = "KYS Killer Color",
                    SpectatorColor = "KYS Spectator Color",
                    GeneratorColor = "KYS Generator Color",
                    HookColor = "KYS Hook Color",
                    GateColor = "KYS Gate Color",
                    WindowColor = "KYS Window Color",
                    PalletColor = "KYS Pallet Color",
                    SCPZombieColor = "KYS SCP Zombie Color",
                }
                for stateKey, flagName in pairs(espFlags) do
                    local elem = Window and Window.ConfigElements and Window.ConfigElements[flagName]
                    if elem and elem.Set and espState[stateKey] ~= nil then
                        pcall(function() elem:Set(espState[stateKey]) end)
                    end
                end

                if getgenv().KYS_RefreshAllPlayers then
                    pcall(getgenv().KYS_RefreshAllPlayers)
                end
            end
...
local function KYS_SaveAndDisable(obj)
    if not obj or KYS_PerfState.saved[obj] ~= nil then return end
    local state = {}
    ...
    if next(state) then KYS_PerfState.saved[obj] = state end
end

local function KYS_ApplyPerformance()
    local enabled = VD.ANTI_LAG == true or VD.FPS_BOOST == true

    if not enabled then
        for obj, state in pairs(KYS_PerfState.saved) do
            if obj and obj.Parent then
                pcall(function() obj.Enabled = state.Enabled end)
            end
            KYS_PerfState.saved[obj] = nil
        end
        if KYS_PerfState.connection then
            pcall(function() KYS_PerfState.connection:Disconnect() end)
            KYS_PerfState.connection = nil
        end
        if KYS_PerfState.qualitySaved ~= nil then
            pcall(function() settings().Rendering.QualityLevel = KYS_PerfState.qualitySaved end)
            KYS_PerfState.qualitySaved = nil
        end
        return
    end

    if VD.FPS_BOOST then
        pcall(function()
            if KYS_PerfState.qualitySaved == nil then
                KYS_PerfState.qualitySaved = settings().Rendering.QualityLevel
            end
    ...
    for _, obj in ipairs(Workspace:GetDescendants()) do
        ...
        or obj:IsA("Smoke") or obj:IsA("Fire") or obj:IsA("Sparkles")
        or obj:IsA("PostEffect") then
            KYS_SaveAndDisable(obj)
        end
    end

    if not KYS_PerfState.connection then
        KYS_PerfState.connection = Workspace.DescendantAdded:Connect(function(obj)
            if VD.ANTI_LAG or VD.FPS_BOOST then
                if obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam")
                or obj:IsA("Smoke") or obj:IsA("Fire") or obj:IsA("Sparkles")
                or obj:IsA("PostEffect") then
                    task.defer(function() KYS_SaveAndDisable(obj) end)
                end
            end
        end)
    end
...
local function KYS_SetAntiLag(v)
    VD.ANTI_LAG = v == true
    KYS_ApplyPerformance()
end

local function KYS_SetFPSBoost(v)
    VD.FPS_BOOST = v == true
    KYS_ApplyPerformance()
end

local function KYS_SetPingBoost(v)
    VD.PING_BOOST = v == true
    if VD.PING_BOOST then
        -- Client-side only. It does not spoof or alter RemoteEvent traffic.
        -- When available, a higher client frame cap can reduce input/render
        -- jitter, but actual server RTT remains unchanged.
        pcall(function()
            if setfpscap then setfpscap(240) end
        end)
        warn("[VD] Ping Boost enabled: client optimization only; server RTT is unchanged.")
    end
end

getgenv().KYS_SetAntiLag = KYS_SetAntiLag
getgenv().KYS_SetFPSBoost = KYS_SetFPSBoost
getgenv().KYS_SetPingBoost = KYS_SetPingBoost
...
if Window then
    ...
    -- =============================================================
    -- ui.lua compatibility adapter
    -- =============================================================
    local function registerConfig(flag, control)
        if flag and flag ~= "" then
            Window.ConfigElements[flag] = control
        end
        return control
    end

    local function pageOf(section)
        return section and section.page
    end

    local function row(section, height)
        local page = pageOf(section)
    ...
    local function text(parent, cfg)
        local l = Instance.new("TextLabel")
        l.BackgroundTransparency = 1
        l.BorderSizePixel = 0
        l.Font = Enum.Font.Gotham
        l.TextSize = 13
        l.TextColor3 = Color3.fromRGB(235, 232, 238)
            ...
            __index = function(_, k)
                if k == "AddSection" then
                    return function(_, cfg)
                        ...
                        return makeModernAdapter(section)
                    end
                end
                ...
                if k == "AddSlider" then
                    return function(_, cfg)
                        cfg = cfg or {}
                        local inc = cfg.Increment
                        if not inc and cfg.Decimals then inc = 10 ^ (-cfg.Decimals) end
                        ...
                        return registerConfig(cfg.Flag or cfg.Name, control)
                    end
                end
                ...
                if k == "AddButton" then
                    return function(_, cfg)
                        cfg = cfg or {}
                        local control = section:AddButton(cfg.Name or "Button", cfg.Callback)
                        return registerConfig(cfg.Flag or cfg.Name, control)
                    end
                end
                ...
                if k == "AddLabel" then
                    return function(_, cfg)
                        cfg = cfg or {}
                        return section:AddLabel(cfg.Name or cfg.Text or "")
                    end
                end

                if k == "AddParagraph" then
                    return function(_, cfg)
                        cfg = cfg or {}
                        return section:AddParagraph(cfg.Name or cfg.Title or "", cfg.Content or cfg.Description or cfg.Text or "")
                    end
                end

                if k == "AddDivider" then
                    return function(_, cfg)
                        cfg = cfg or {}
                        return section:AddDivider({Text = cfg.Text or cfg.Name or ""})
                    end
                end

                if k == "AddTextInput" then
                    return function(_, cfg)
                        cfg = cfg or {}
                        local control = section:AddTextInput({
                ...
                if k == "AddDropdown" then
                    return function(_, cfg)
                        ...
                        local r = row(section, 39)
                        text(r, {
                            Size = UDim2.new(1, -160, 1, 0),
                            Text = tostring(cfg.Name or "Select")
                        })
                        ...
                        local arrow = text(b, {
                            ...
                            TextColor3 = Color3.fromRGB(170, 165, 178),
                        ...
                        popup.Visible = false
                        popup.ZIndex = 100
                        popup.Position = UDim2.new(1, -150, 1, 3)
                        popup.Size = UDim2.fromOffset(150, math.min(190, math.max(38, #values * 30 + 8)))
                        popup.BackgroundColor3 = Color3.fromRGB(20, 19, 24)
                        popup.BorderSizePixel = 0
                        popup.Parent = r
                        corner(popup, 6)

                        local ps = Instance.new("UIStroke")
                        ps.Color = Color3.fromRGB(213, 164, 203)
                        ps.Transparency = 0.55
                        ps.Parent = popup

                        local sc = Instance.new("ScrollingFrame")
                        sc.Size = UDim2.new(1, -8, 1, -8)
                        sc.Position = UDim2.fromOffset(4, 4)
                        sc.BackgroundTransparency = 1
                        sc.BorderSizePixel = 0
                        sc.ScrollBarThickness = 2
                        sc.ScrollBarImageColor3 = Color3.fromRGB(213, 164, 203)
                        ...
                        local function summary()
                            if not multi then
                                return selected == nil and "Select..." or tostring(selected)
                            end
                            local list = {}
                        ...
                        local function fire()
                            ...
                            if multi then
                                ...
                                task.spawn(function() pcall(cfg.Callback, list) end)
                            else
                                task.spawn(function() pcall(cfg.Callback, selected) end)
                            end
                        end
                        ...
                        local function refresh()
                            ...
                            for value, item in pairs(items) do
                                local active = multi and selected[value] or (selected == value)
                                item.BackgroundColor3 = active and Color3.fromRGB(55, 35, 54) or Color3.fromRGB(25, 24, 29)
                                item.TextColor3 = active and Color3.fromRGB(255, 235, 252) or Color3.fromRGB(215, 210, 220)
                            end
                        end
                        ...
                        for _, value in ipairs(values) do
                            ...
                            item.BackgroundColor3 = Color3.fromRGB(25, 24, 29)
                            ...
                            item.TextColor3 = Color3.fromRGB(215, 210, 220)
                            item.Parent = sc
                            corner(item, 4)
                ...
                if k == "AddColorPicker" then
                    return function(_, cfg)
                        cfg = cfg or {}
                        local current = typeof(cfg.Default) == "Color3" and cfg.Default or Color3.fromRGB(255, 255, 255)

                        local r = row(section, 39)
                        text(r, {
                            Size = UDim2.new(1, -60, 1, 0),
                            Text = tostring(cfg.Name or "Color")
                        })

                        local swatch = Instance.new("TextButton")
                        swatch.AnchorPoint = Vector2.new(1, 0.5)
                        swatch.Position = UDim2.new(1, 0, 0.5, 0)
                        swatch.Size = UDim2.fromOffset(42, 24)
                        ...
                        outline.Color = Color3.fromRGB(245, 242, 247)
                        ...
                        popup.Size = UDim2.fromOffset(190, 132)
                        popup.BackgroundColor3 = Color3.fromRGB(20, 19, 24)
                        popup.BorderSizePixel = 0
                        popup.Parent = r
                        corner(popup, 6)
                        ...
                        pst.Color = Color3.fromRGB(213, 164, 203)
                        pst.Transparency = 0.55
                        pst.Parent = popup

                        local preview = Instance.new("Frame")
                        preview.Position = UDim2.fromOffset(10, 9)
                        preview.Size = UDim2.fromOffset(170, 22)
                        ...
                        for i, name in ipairs({"R", "G", "B"}) do
                            ...
                            text(popup, {
                                ...
                                Text = name,
                                TextSize = 11,
                                TextColor3 = Color3.fromRGB(180, 175, 186)
                            })

                            local bar = Instance.new("TextButton")
                            bar.Position = UDim2.fromOffset(32, y + 7)
                            bar.Size = UDim2.fromOffset(145, 7)
                            bar.BackgroundColor3 = Color3.fromRGB(55, 52, 60)
                            ...
                            fill.BackgroundColor3 = current
                            fill.BorderSizePixel = 0
                            fill.Parent = bar
                            corner(fill, 4)
                            channels[i] = {bar = bar, fill = fill}
                        end

                        local function rgb()
                            return math.floor(current.R * 255 + 0.5),
                        ...
                        local function redraw()
                            ...
                            local values = {rgb()}
                            for i = 1, 3 do
                                channels[i].fill.Size = UDim2.new(values[i] / 255, 0, 1, 0)
                                channels[i].fill.BackgroundColor3 = current
                            end
                        end

                        local dragging
                        local function setChannel(i, input)
                            local bar = channels[i].bar
                            local ratio = math.clamp(
                                (input.Position.X - bar.AbsolutePosition.X) / math.max(1, bar.AbsoluteSize.X),
                                0, 1
                            )
                            local r8, g8, b8 = rgb()
                            local vals = {r8, g8, b8}
                            vals[i] = math.floor(ratio * 255 + 0.5)
                            current = Color3.fromRGB(vals[1], vals[2], vals[3])
                            redraw()
                        ...
                        for i = 1, 3 do
                            channels[i].bar.InputBegan:Connect(function(input)
                                if input.UserInputType == Enum.UserInputType.MouseButton1
                ...
                if k == "AddKeybind" then
                    return function(_, cfg)
                        cfg = cfg or {}
                        local r = row(section, 36)
                        text(r, {
                        ...
                        b.AnchorPoint = Vector2.new(1, 0.5)
                        b.Position = UDim2.new(1, 0, 0.5, 0)
                        b.Size = UDim2.fromOffset(105, 26)
                        b.BackgroundColor3 = Color3.fromRGB(27, 25, 30)
                        b.BorderSizePixel = 0
                        b.AutoButtonColor = false
                        b.Font = Enum.Font.Gotham
                        b.TextSize = 11
                        b.TextColor3 = Color3.fromRGB(225, 220, 230)
                        b.Text = tostring(cfg.Default or "None")
                        b.Parent = r
                        corner(b, 5)
                        ...
                        UserInputService.InputBegan:Connect(function(input, processed)
                            ...
                            if cfg.Callback then
                                task.spawn(function() pcall(cfg.Callback, input.KeyCode) end)
                            end
                        end)

                        return registerConfig(cfg.Flag or cfg.Name, {
                            Get = function() return b.Text end,
                            Set = function(_, v) b.Text = tostring(v) end
                        })
                    end
                end
    ...
    -- ui.lua tidak punya nested Tabbox/AddTab. Setiap "feature tab" dibuat
    -- sebagai page nyata di Window agar seluruh callback fitur tetap dibangun.
    local function addCenterFeatureTabbox(_tab, _name, entries)
        local created = {}
        for _, entry in ipairs(entries) do
            created[entry.Key] = makeModernAdapter(Window:tab(entry.Name))
        end
        return created
    end
    ...
    local utilitySection = MainKillerFeatureTabs.Utilities:AddSection({
        Position = "Center",
    ...
    utilitySection:AddToggle({
        ...
        Flag = "Anti Lag",
        Default = VD.ANTI_LAG,
        Callback = function(v) KYS_SetAntiLag(v) end
    })

    utilitySection:AddToggle({
        Name = "FPS Boost",
        Flag = "FPS Boost",
        Default = VD.FPS_BOOST,
        Callback = function(v) KYS_SetFPSBoost(v) end
    })

    utilitySection:AddToggle({
        Name = "Ping Boost",
        Flag = "Ping Boost",
        Default = VD.PING_BOOST,
        Callback = function(v) KYS_SetPingBoost(v) end
    })

    utilitySection:AddDivider({Text = "Config"})
    ...
    utilitySection:AddTextInput({
        ...
        Callback = function(v)
            if v and v ~= "" then getgenv().CurrentConfigName = tostring(v) end
        end
    })
    ...
    utilitySection:AddButton({
        Name = "Save Config",
        Callback = function()
            KYS_SaveConfig(getgenv().CurrentConfigName or "Default")
        end
    })
    ...
    utilitySection:AddButton({
        Name = "Load Config",
        Callback = function()
            KYS_LoadConfig(getgenv().CurrentConfigName or "Default")
            pcall(KYS_ApplyPerformance)
        end
    })

    utilitySection:AddDropdown({
        Name = "Load Existing Config",
        Flag = "Load Existing Config",
        ...
        Callback = function(v)
            ...
            if v and v ~= "" then
                getgenv().CurrentConfigName = tostring(v)
                KYS_LoadConfig(v)
                pcall(KYS_ApplyPerformance)
            end
        end
