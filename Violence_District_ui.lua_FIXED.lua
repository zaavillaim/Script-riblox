...
-- =====================================================
-- UI.LUA MODERN MOBILE UI
-- Uses the uploaded ui.lua as the actual renderer.
-- =====================================================
local ModernV2
do
    local __ui_src = [=[
...
return k
    ...
    local __ui_loader = loadstring or load
    local __ok, __lib = pcall(function()
        local fn = __ui_loader(__ui_src)
        if not fn then error("ui.lua compile failed") end
        return fn()
    end)
    if __ok and type(__lib) == "table" then
        ModernV2 = __lib
    else
        warn("[KysHub] ui.lua failed to load:", __lib)
    end
end

local function __ui_round(inst, radius)
...
local function __ui_make_adapter(page)
    local adapter = {}
    local rawPage = page.page
    local rootGui = page.gui
    function adapter:AddSection(cfg) if cfg and cfg.Name then page:section(cfg.Name) end return adapter end
    function adapter:AddDivider(cfg) if cfg and cfg.Text then page:section(cfg.Text) end return adapter end
    function adapter:AddToggle(cfg)
        cfg=cfg or {}
        local obj=page:toggle(cfg.Name or "Toggle",cfg.Default==true,function(v) if cfg.Callback then pcall(cfg.Callback,v) end end)
        if cfg.Flag then Window.ConfigElements[cfg.Flag]=obj end
        return obj
    end
    function adapter:AddSlider(cfg)
        cfg=cfg or {}
        local obj=page:slider(cfg.Name or "Slider",tonumber(cfg.Min) or 0,tonumber(cfg.Max) or 100,tonumber(cfg.Default) or tonumber(cfg.Min) or 0,function(v) if cfg.Callback then pcall(cfg.Callback,v) end end)
        if cfg.Flag then Window.ConfigElements[cfg.Flag]=obj end
        return obj
    end
    function adapter:AddButton(cfg) cfg=cfg or {}; return page:button(cfg.Name or cfg.Text or "Button",function() if cfg.Callback then pcall(cfg.Callback) end end) end
    function adapter:AddTextInput(cfg)
        cfg=cfg or {}
        local obj=page:input(cfg.Name or "Input",cfg.Placeholder or cfg.Default or "",function(text,enter) if cfg.Callback then pcall(cfg.Callback,text,enter) end end)
        if cfg.Default~=nil then obj:Set(cfg.Default) end
        if cfg.Flag then Window.ConfigElements[cfg.Flag]=obj end
        return obj
    end
    function adapter:AddKeybind(cfg)
        cfg=cfg or {}; local current=cfg.Default or cfg.Key or "None"
        local obj=page:button((cfg.Name or "Keybind").."  ["..tostring(current).."]",function() if cfg.Callback then pcall(cfg.Callback,current) end end)
        if cfg.Flag then Window.ConfigElements[cfg.Flag]=obj end
        return obj
    end
    function adapter:AddDropdown(cfg)
        cfg=cfg or {}; local values=cfg.Values or {}; local multi=cfg.Multi==true; local selected={}; local current=cfg.Default
        if multi then if type(current)=="table" then for _,v in ipairs(current) do selected[tostring(v)]=true end elseif current~=nil then selected[tostring(current)]=true end else if type(current)=="table" then current=current[1] end end
        local row=Instance.new("Frame"); row.Size=UDim2.new(1,0,0,46); row.BackgroundTransparency=1; row.Parent=rawPage
        local title=Instance.new("TextLabel"); title.Size=UDim2.new(.42,0,1,0); title.BackgroundTransparency=1; title.Text=tostring(cfg.Name or "Select"); title.TextColor3=Color3.fromRGB(235,232,238); title.Font=Enum.Font.Gotham; title.TextSize=13; title.TextXAlignment=Enum.TextXAlignment.Left; title.Parent=row
        local button=Instance.new("TextButton"); button.AnchorPoint=Vector2.new(1,.5); button.Position=UDim2.new(1,0,.5,0); button.Size=UDim2.new(.56,0,0,30); button.BackgroundColor3=Color3.fromRGB(27,25,30); button.TextColor3=Color3.fromRGB(235,232,238); button.Font=Enum.Font.Gotham; button.TextSize=11; button.TextTruncate=Enum.TextTruncate.AtEnd; button.AutoButtonColor=false; button.Parent=row; __ui_round(button,6)
        local popup=Instance.new("Frame"); popup.Visible=false; popup.BackgroundColor3=Color3.fromRGB(18,17,21); popup.BorderSizePixel=0; popup.ZIndex=100; popup.Parent=rootGui; __ui_round(popup,6)
        local list=Instance.new("ScrollingFrame"); list.Size=UDim2.new(1,-8,1,-8); list.Position=UDim2.fromOffset(4,4); list.BackgroundTransparency=1; list.BorderSizePixel=0; list.ScrollBarThickness=2; list.ScrollBarImageColor3=Color3.fromRGB(213,164,203); list.AutomaticCanvasSize=Enum.AutomaticSize.Y; list.ZIndex=101; list.Parent=popup
        local lay=Instance.new("UIListLayout"); lay.Padding=UDim.new(0,3); lay.Parent=list
        local function text() if multi then local out={}; for _,v in ipairs(values) do if selected[tostring(v)] then table.insert(out,tostring(v)) end end; return #out>0 and table.concat(out,", ") or "None" end return current~=nil and tostring(current) or "None" end
        local function refresh() button.Text=text(); for _,ch in ipairs(list:GetChildren()) do if ch:IsA("TextButton") then local v=ch:GetAttribute("Value"); ch.Text=((multi and selected[tostring(v)]) and "✓  " or "    ")..tostring(v) end end end
        for _,v in ipairs(values) do
            local opt=Instance.new("TextButton"); opt.Size=UDim2.new(1,0,0,28); opt.BackgroundColor3=Color3.fromRGB(31,28,35); opt.TextColor3=Color3.fromRGB(235,232,238); opt.Font=Enum.Font.Gotham; opt.TextSize=11; opt.TextXAlignment=Enum.TextXAlignment.Left; opt.TextTruncate=Enum.TextTruncate.AtEnd; opt.AutoButtonColor=false; opt:SetAttribute("Value",tostring(v)); opt.Parent=list; __ui_round(opt,5)
            opt.Activated:Connect(function() if multi then local key=tostring(v); selected[key]=not selected[key]; local result={}; for _,vv in ipairs(values) do if selected[tostring(vv)] then table.insert(result,vv) end end; if cfg.Callback then pcall(cfg.Callback,result) end else current=v; popup.Visible=false; if cfg.Callback then pcall(cfg.Callback,v) end end; refresh() end)
        end
        local function pos() if popup.Visible then local ap=button.AbsolutePosition; local as=button.AbsoluteSize; popup.Size=UDim2.fromOffset(math.max(170,as.X),math.min(7,#values)*31+8); local cam=workspace.CurrentCamera; local vp=cam and cam.ViewportSize; local y=ap.Y+as.Y+3; if vp and y+popup.AbsoluteSize.Y>vp.Y then y=ap.Y-popup.AbsoluteSize.Y-3 end; popup.Position=UDim2.fromOffset(ap.X,y) end end
        button.Activated:Connect(function() popup.Visible=not popup.Visible; pos() end); rootGui:GetPropertyChangedSignal("AbsoluteSize"):Connect(pos); refresh()
        local obj={Get=function() if multi then local r={}; for _,v in ipairs(values) do if selected[tostring(v)] then table.insert(r,v) end end; return r end; return current end, Set=function(_,v) if multi then selected={}; if type(v)=="table" then for _,vv in ipairs(v) do selected[tostring(vv)]=true end elseif v~=nil then selected[tostring(v)]=true end else if type(v)=="table" then v=v[1] end; current=v end; refresh() end}
        if cfg.Flag then Window.ConfigElements[cfg.Flag]=obj end; return obj
    end
    function adapter:AddColorPicker(cfg)
        cfg=cfg or {}; local color=typeof(cfg.Default)=="Color3" and cfg.Default or Color3.fromRGB(255,255,255); local palette={Color3.fromRGB(235,70,70),Color3.fromRGB(255,170,0),Color3.fromRGB(255,235,70),Color3.fromRGB(80,220,120),Color3.fromRGB(80,170,255),Color3.fromRGB(170,100,255),Color3.fromRGB(255,110,190),Color3.fromRGB(245,245,245)}
        local row=Instance.new("Frame"); row.Size=UDim2.new(1,0,0,38); row.BackgroundTransparency=1; row.Parent=rawPage
        local title=Instance.new("TextLabel"); title.Size=UDim2.new(.7,0,1,0); title.BackgroundTransparency=1; title.Text=tostring(cfg.Name or "Color"); title.TextColor3=Color3.fromRGB(235,232,238); title.Font=Enum.Font.Gotham; title.TextSize=13; title.TextXAlignment=Enum.TextXAlignment.Left; title.Parent=row
        local sw=Instance.new("TextButton"); sw.AnchorPoint=Vector2.new(1,.5); sw.Position=UDim2.new(1,0,.5,0); sw.Size=UDim2.fromOffset(44,24); sw.BackgroundColor3=color; sw.Text=""; sw.AutoButtonColor=false; sw.Parent=row; __ui_round(sw,7)
        local function set(c,call) color=c; sw.BackgroundColor3=c; if call and cfg.Callback then pcall(cfg.Callback,c) end end
        sw.Activated:Connect(function() local idx=1; local best=math.huge; for i,c in ipairs(palette) do local dr=color.R-c.R; local dg=color.G-c.G; local db=color.B-c.B; local d=dr*dr+dg*dg+db*db; if d<best then best=d; idx=i end end; set(palette[(idx%#palette)+1],true) end)
        local obj={Get=function() return color end,Set=function(_,v) if typeof(v)=="Color3" then set(v,true) end end}; if cfg.Flag then Window.ConfigElements[cfg.Flag]=obj end; return obj
    end
    function adapter:AddParagraph(cfg) cfg=cfg or {}; return page:label(cfg.Content or cfg.Text or cfg.Name or "") end
    function adapter:AddLabel(cfg) cfg=cfg or {}; return page:label(cfg.Text or cfg.Name or "") end
    return setmetatable(adapter,{__index=function(_,k) return page[k] end})
end

if ModernV2 then
    function ModernV2:AddTheme(_) end
    function ModernV2:CreateMenuIcon(_) return nil end
    function ModernV2:Window(cfg)
        cfg=cfg or {}; local root=ModernV2.new(cfg.Title or "Violence District"); root.ConfigElements={}; root.__root=root; root.__pages={}
        local function makePage(name) local p=root:tab(name); p.gui=root.gui; return __ui_make_adapter(p) end
        function root:AddTab(c) return makePage((type(c)=="table" and c.Name) or tostring(c or "Tab")) end
        function root:CreateHomeTab(_) end; function root:SetAccount(_) end; function root:AttachMenuIcon(_) end
        function root:Notify(_) end
        return root
    end
    Window=ModernV2:Window({Title="Violence District",Content="Modern UI",Size=UDim2.fromOffset(470,310)})
end
...
if Window then
    local function adaptTab(tab) return tab end
    local function addCenterFeatureTabbox(_, _, entries)
        local created={}
        for _,entry in ipairs(entries) do
            created[entry.Key]=Window:AddTab({Name=entry.Name,Icon=entry.Icon,Type="Single"})
        end
        return created
    end
    local Tabs = {
    ...
    VisualFeatureTabs=addCenterFeatureTabbox(Tabs.Visual,"Visual Features",{{{Key="ESP",Name="ESP",Icon="lucide:eye"},{Key="Camera",Name="Camera",Icon="solar:camera-bold"},{Key="Lighting",Name="Lighting",Icon="solar:sun-bold"}}})
    MainFeatureTabs=addCenterFeatureTabbox(Tabs.Main,"Main Features",{{{Key="Survivor",Name="Survivor",Icon="solar:shield-bold"},{Key="Escape",Name="Escape",Icon="solar:exit-bold"},{Key="Automation",Name="Automation",Icon="solar:bolt-bold"}}})
    MainKillerFeatureTabs=addCenterFeatureTabbox(Tabs.Main,"Killer & Utilities",{{{Key="Killer",Name="Killer",Icon="solar:danger-bold"},{Key="Ability",Name="Killer Ability",Icon="solar:bolt-bold"},{Key="Utilities",Name="Utilities",Icon="solar:settings-bold"}}})
    AimFeatureTabs=addCenterFeatureTabbox(Tabs.Aim,"Aim Features",{{{Key="Aimbot",Name="Aimbot",Icon="solar:target-bold"},{Key="Spear",Name="Killer Aim",Icon="lucide:sword"},{Key="AutoAim",Name="Survivor Aim",Icon="solar:magic-stick-3-bold"}}})
    MappingFeatureTabs=addCenterFeatureTabbox(Tabs.Mapping,"Mapping Features",{{{Key="Teleport",Name="Teleport",Icon="solar:map-point-bold"},{Key="Radar",Name="Radar",Icon="solar:radar-bold"}}})
    PlayerFeatureTabs=addCenterFeatureTabbox(Tabs.Player,"Player Features",{{{Key="Movement",Name="Movement",Icon="solar:running-round-bold"},{Key="Fling",Name="Fling",Icon="solar:wind-bold"},{Key="Emote",Name="Emote [BETA]",Icon="solar:music-note-bold"}}})
    PlayerMiscFeatureTabs=addCenterFeatureTabbox(Tabs.Player,"Player Misc",{{{Key="Fun",Name="Fun",Icon="solar:gamepad-bold"},{Key="Streamer",Name="Streamer Mode",Icon="solar:settings-bold"},{Key="Avatar",Name="Avatar Tools",Icon="solar:users-group-rounded-bold"}}})
    VisualTab=VisualFeatureTabs.ESP; MainTab=Tabs.Main; AimTab=AimFeatureTabs.Aimbot; MappingTab=MappingFeatureTabs.Teleport; PlayerTab=Tabs.Player
end
