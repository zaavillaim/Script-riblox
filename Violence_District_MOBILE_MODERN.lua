...
-- =====================================================
-- STANDALONE UI
-- No external UI framework / require / HttpGet dependency.
-- Keeps the existing feature callbacks and config flags intact.
-- =====================================================
...
local function stroke(o, color, thick)
    mk("UIStroke", {Color = color or Color3.fromRGB(65,68,76), Thickness = thick or 1, Transparency = 0.15}, o)
end
...
function StandaloneUI:Window(cfg)
    local self = { ConfigElements = {}, Tabs = {}, _active = nil }
    local parent = uiParent()
    local old = parent:FindFirstChild("KysHub_Standalone_VD")
    if old then pcall(function() old:Destroy() end) end

    local accent = cfg.Color or Color3.fromRGB(235,70,70)
    local gui = mk("ScreenGui", {Name="KysHub_Standalone_VD", ResetOnSpawn=false, IgnoreGuiInset=true, ZIndexBehavior=Enum.ZIndexBehavior.Sibling}, parent)
    self.Gui = gui

    local mobile = UserInputService.TouchEnabled
    local windowSize = mobile and UDim2.fromScale(0.94, 0.78) or (cfg.Size or UDim2.fromOffset(700,480))
    local windowPos = mobile and UDim2.fromScale(0.03, 0.10) or UDim2.new(0.5,-350,0.5,-240)
    local shadow = mk("Frame", {BackgroundColor3=Color3.fromRGB(0,0,0), BackgroundTransparency=0.55, BorderSizePixel=0, Size=windowSize, Position=windowPos}, gui)
    corner(shadow, 10)
    local main = mk("Frame", {BackgroundColor3=Color3.fromRGB(17,18,22), BackgroundTransparency=KysUI_Solid and 0 or 0.04, BorderSizePixel=0, Size=UDim2.fromScale(1,1)}, shadow)
    corner(main, 10); stroke(main, Color3.fromRGB(60,62,70), 1)

    local top = mk("Frame", {BackgroundColor3=Color3.fromRGB(21,22,27), BorderSizePixel=0, Size=UDim2.new(1,0,0,mobile and 58 or 46)}, main)
    corner(top, 10)
    local title = label(top, cfg.Title or "Standalone UI", 15, true); title.Position=UDim2.fromOffset(16,0); title.Size=UDim2.new(1,-70,0,34)
    local subtitle = label(top, cfg.Content or "", 11, false); subtitle.TextColor3=Color3.fromRGB(145,148,158); subtitle.Position=UDim2.fromOffset(16,31); subtitle.Size=UDim2.new(1,-70,0,18)
    local close = mk("TextButton", {BackgroundTransparency=1, Text="×", TextColor3=Color3.fromRGB(210,212,218), TextSize=26, Font=Enum.Font.GothamBold, Size=UDim2.fromOffset(48,48), Position=UDim2.new(1,-52,0,5)}, top)
    close.MouseButton1Click:Connect(function() main.Visible=false end)
    if mobile then
        local reopen = mk("TextButton", {BackgroundColor3=Color3.fromRGB(21,22,27), Text="VD", TextColor3=accent, TextSize=14, Font=Enum.Font.GothamBold, Size=UDim2.fromOffset(52,52), Position=UDim2.new(1,-64,1,-72), Visible=false, AutoButtonColor=false}, gui)
        corner(reopen, 16); stroke(reopen, accent, 1)
        close.MouseButton1Click:Connect(function() reopen.Visible=true end)
        reopen.MouseButton1Click:Connect(function() main.Visible=true; reopen.Visible=false end)
    end

    local navHeight = 0
    local sidebar = mk("ScrollingFrame", {BackgroundColor3=Color3.fromRGB(14,15,18), BorderSizePixel=0,
        Size=mobile and UDim2.new(0,230,1,-58) or UDim2.new(0,150,1,-46),
        Position=mobile and UDim2.fromOffset(-230,58) or UDim2.fromOffset(0,46),
        ScrollBarThickness=mobile and 0 or 3, AutomaticCanvasSize=Enum.AutomaticSize.Y,
        CanvasSize=UDim2.new(), ScrollingDirection=Enum.ScrollingDirection.Y, ZIndex=30}, main)
    corner(sidebar,8)
    local sideLayout = mk("UIListLayout", {Padding=UDim.new(0,6), SortOrder=Enum.SortOrder.LayoutOrder}, sidebar)
    mk("UIPadding", {PaddingTop=UDim.new(0,12), PaddingLeft=UDim.new(0,10), PaddingRight=UDim.new(0,10), PaddingBottom=UDim.new(0,14)}, sidebar)

    local content = mk("Frame", {BackgroundTransparency=1,
        Size=mobile and UDim2.new(1,0,1,-58) or UDim2.new(1,-150,1,-46),
        Position=mobile and UDim2.fromOffset(0,58) or UDim2.new(0,150,0,46)}, main)
    ...
    local function setDrawer(open)
        ...
        drawerOpen=open
        sidebar:TweenPosition(open and UDim2.fromOffset(0,58) or UDim2.fromOffset(-230,58), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, 0.20, true)
        if drawerShade then drawerShade.Visible=open end
    end
    if mobile then
        drawerShade=mk("TextButton", {BackgroundColor3=Color3.fromRGB(0,0,0), BackgroundTransparency=0.45, BorderSizePixel=0,
            Text="", AutoButtonColor=false, Size=UDim2.new(1,0,1,-58), Position=UDim2.fromOffset(0,58), Visible=false, ZIndex=25}, main)
        drawerShade.MouseButton1Click:Connect(function() setDrawer(false) end)
        local menu=mk("TextButton", {BackgroundColor3=Color3.fromRGB(30,31,37), BorderSizePixel=0, Text="☰", TextColor3=accent,
            TextSize=22, Font=Enum.Font.GothamBold, Size=UDim2.fromOffset(42,42), Position=UDim2.fromOffset(8,8), AutoButtonColor=false, ZIndex=12}, top)
        corner(menu,10); stroke(menu,Color3.fromRGB(65,68,76),1)
        menu.MouseButton1Click:Connect(function() setDrawer(not drawerOpen) end)
        title.Position=UDim2.fromOffset(58,0); subtitle.Position=UDim2.fromOffset(58,31)
        title.Size=UDim2.new(1,-120,0,34); subtitle.Size=UDim2.new(1,-120,0,18)
    end
    ...
    top.InputBegan:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
            dragging=true; dragStart=i.Position; startPos=shadow.Position
        end
    end)
    ...
    function self:AddTab(tcfg)
        local tab={Name=tcfg.Name or "Tab", Window=self, Sections={}, Pages={}}
        local b=mk("TextButton", {BackgroundColor3=Color3.fromRGB(22,23,28), BorderSizePixel=0, Text=mobile and ("   "..tab.Name) or ("  "..tab.Name),
            TextColor3=Color3.fromRGB(175,178,188), TextSize=mobile and 13 or 12, Font=Enum.Font.GothamSemibold,
            TextXAlignment=Enum.TextXAlignment.Left, Size=mobile and UDim2.new(1,0,0,46) or UDim2.new(1,0,0,34), AutoButtonColor=false}, sidebar)
        corner(b,8)
        tab.Button=b
        local page=mk("ScrollingFrame", {BackgroundTransparency=1, BorderSizePixel=0, Size=UDim2.fromScale(1,1), ScrollBarThickness=4, AutomaticCanvasSize=Enum.AutomaticSize.Y, CanvasSize=UDim2.new()}, content)
        mk("UIPadding", {PaddingTop=UDim.new(0,10), PaddingLeft=UDim.new(0,12), PaddingRight=UDim.new(0,12), PaddingBottom=UDim.new(0,18)}, page)
        mk("UIListLayout", {Padding=UDim.new(0,8), SortOrder=Enum.SortOrder.LayoutOrder}, page)
        tab.Page=page
        function tab:AddDivider(dcfg)
            local l=label(page, dcfg and dcfg.Text or "", 12, true); l.Size=UDim2.new(1,0,0,24); l.TextColor3=accent; return l
        end
        function tab:AddSection(scfg)
            local sec={Name=scfg and scfg.Name or "Section", Root=mk("Frame", {BackgroundColor3=Color3.fromRGB(21,22,27), BorderSizePixel=0, AutomaticSize=Enum.AutomaticSize.Y, Size=UDim2.new(1,0,0,0)}, page), Elements={}}
            corner(sec.Root,8); stroke(sec.Root,Color3.fromRGB(54,56,64),1)
            local pad=mk("UIPadding", {PaddingTop=UDim.new(0,8),PaddingBottom=UDim.new(0,10),PaddingLeft=UDim.new(0,10),PaddingRight=UDim.new(0,10)}, sec.Root)
            local lay=mk("UIListLayout", {Padding=UDim.new(0,6),SortOrder=Enum.SortOrder.LayoutOrder}, sec.Root)
            local h=label(sec.Root,sec.Name,12,true); h.Size=UDim2.new(1,0,0,22); h.TextColor3=Color3.fromRGB(245,245,247)
            local function reg(flag, elem) if flag then self.ConfigElements[flag]=elem end end
            function sec:AddToggle(ecfg)
                local state=ecfg.Default==true
                local row=mk("TextButton", {BackgroundColor3=Color3.fromRGB(27,28,34), BorderSizePixel=0, Text="", Size=UDim2.new(1,0,0,34), AutoButtonColor=false}, sec.Root); corner(row,6)
                local tx=label(row,ecfg.Name or "Toggle",12,false); tx.Position=UDim2.fromOffset(10,0); tx.Size=UDim2.new(1,-58,1,0)
                local pill=mk("Frame", {BackgroundColor3=Color3.fromRGB(65,67,76), BorderSizePixel=0, Size=UDim2.fromOffset(36,18), Position=UDim2.new(1,-46,0.5,-9)}, row); corner(pill,9)
                local knob=mk("Frame", {BackgroundColor3=Color3.fromRGB(220,220,225), BorderSizePixel=0, Size=UDim2.fromOffset(14,14), Position=UDim2.fromOffset(2,2)}, pill); corner(knob,7)
                local elem={Value=state}
                local function apply(v, fire) state=not not v; elem.Value=state; pill.BackgroundColor3=state and accent or Color3.fromRGB(65,67,76); knob.Position=state and UDim2.fromOffset(20,2) or UDim2.fromOffset(2,2); if fire and ecfg.Callback then pcall(ecfg.Callback,state) end end
                function elem:Set(v) apply(v,true) end
                row.MouseButton1Click:Connect(function() apply(not state,true) end)
                apply(state,false); reg(ecfg.Flag or ecfg.Name,elem); return elem
            end
            function sec:AddSlider(ecfg)
                local row=mk("Frame", {BackgroundTransparency=1, Size=UDim2.new(1,0,0,48)}, sec.Root)
                local tx=label(row,ecfg.Name or "Slider",12,false); tx.Size=UDim2.new(1,-70,0,22)
                local val=label(row,tostring(ecfg.Default or ecfg.Min or 0),11,true); val.TextXAlignment=Enum.TextXAlignment.Right; val.Position=UDim2.new(1,-60,0,0); val.Size=UDim2.fromOffset(60,22); val.TextColor3=accent
                local bar=mk("Frame", {BackgroundColor3=Color3.fromRGB(52,54,62), BorderSizePixel=0, Position=UDim2.fromOffset(0,30), Size=UDim2.new(1,0,0,5)}, row); corner(bar,3)
                local fill=mk("Frame", {BackgroundColor3=accent, BorderSizePixel=0, Size=UDim2.new(0,0,1,0)}, bar); corner(fill,3)
                local min,max=tonumber(ecfg.Min or 0),tonumber(ecfg.Max or 100); local value=tonumber(ecfg.Default or min) or min
                local elem={Value=value}
                local function set(v,fire) value=math.clamp(tonumber(v) or min,min,max); elem.Value=value; local a=(value-min)/(max-min); fill.Size=UDim2.new(a,0,1,0); val.Text=string.format("%s", tostring(math.floor(value*100+0.5)/100)); if fire and ecfg.Callback then pcall(ecfg.Callback,value) end end
                function elem:Set(v) set(v,true) end
                local active=false
                bar.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then active=true; set(min+(max-min)*math.clamp((i.Position.X-bar.AbsolutePosition.X)/bar.AbsoluteSize.X,0,1),true) end end)
                UserInputService.InputChanged:Connect(function(i) if active and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then set(min+(max-min)*math.clamp((i.Position.X-bar.AbsolutePosition.X)/bar.AbsoluteSize.X,0,1),true) end end)
                ...
                set(value,false); reg(ecfg.Flag or ecfg.Name,elem); return elem
            end
            function sec:AddDropdown(ecfg)
                local values=ecfg.Values or {}; local current=ecfg.Default or values[1] or ""
                local b=mk("TextButton", {BackgroundColor3=Color3.fromRGB(27,28,34),BorderSizePixel=0,Text="",Size=UDim2.new(1,0,0,38),AutoButtonColor=false},sec.Root);corner(b,6)
                local tx=label(b,ecfg.Name or "Dropdown",12,false);tx.Position=UDim2.fromOffset(10,0);tx.Size=UDim2.new(0.5,0,1,0)
                local vv=label(b,tostring(current),11,true);vv.TextXAlignment=Enum.TextXAlignment.Right;vv.Position=UDim2.new(0.5,0,0,0);vv.Size=UDim2.new(0.5,-28,1,0);vv.TextColor3=accent
                local elem={Value=current}
                function elem:Set(v) current=v; elem.Value=v; vv.Text=tostring(v); if ecfg.Callback then pcall(ecfg.Callback,v) end end
                function elem:SetValues(v) values=v or {}; if #values>0 then elem:Set(values[1]) end end
                b.MouseButton1Click:Connect(function()
                    local menu=mk("Frame",{BackgroundColor3=Color3.fromRGB(24,25,30),BorderSizePixel=0,Size=UDim2.new(1,0,0,math.min(220,#values*30)),Position=UDim2.new(0,0,1,2),ZIndex=20},b);corner(menu,6);stroke(menu,Color3.fromRGB(65,68,76),1)
                    mk("UIListLayout",{SortOrder=Enum.SortOrder.LayoutOrder},menu)
                    for _,v in ipairs(values) do local ob=mk("TextButton",{BackgroundTransparency=1,Text=tostring(v),TextColor3=Color3.fromRGB(220,222,228),TextSize=11,Font=Enum.Font.Gotham,Size=UDim2.new(1,0,0,30),ZIndex=21},menu);ob.MouseButton1Click:Connect(function() elem:Set(v);menu:Destroy() end) end
                    task.delay(5,function() if menu and menu.Parent then menu:Destroy() end end)
                end)
                reg(ecfg.Flag or ecfg.Name,elem); return elem
            end
            function sec:AddButton(ecfg)
                local b=mk("TextButton",{BackgroundColor3=Color3.fromRGB(31,32,39),BorderSizePixel=0,Text=ecfg.Name or "Button",TextColor3=Color3.fromRGB(235,235,240),TextSize=12,Font=Enum.Font.GothamSemibold,Size=UDim2.new(1,0,0,34),AutoButtonColor=false},sec.Root);corner(b,6);b.MouseButton1Click:Connect(function() if ecfg.Callback then pcall(ecfg.Callback) end end);return b
            end
            function sec:AddColorPicker(ecfg)
                local b=mk("TextButton",{BackgroundColor3=ecfg.Default or Color3.fromRGB(255,255,255),BorderSizePixel=0,Text=ecfg.Name or "Color",TextColor3=Color3.fromRGB(20,20,22),TextSize=11,Font=Enum.Font.GothamSemibold,Size=UDim2.new(1,0,0,30),AutoButtonColor=false},sec.Root);corner(b,6)
                local elem={Value=ecfg.Default}
                function elem:Set(v) elem.Value=v;b.BackgroundColor3=v;if ecfg.Callback then pcall(ecfg.Callback,v) end end
                b.MouseButton1Click:Connect(function() end)
                reg(ecfg.Flag or ecfg.Name,elem);return elem
            end
            function sec:AddTextInput(ecfg)
                local row=mk("Frame",{BackgroundTransparency=1,Size=UDim2.new(1,0,0,40)},sec.Root);local box=mk("TextBox",{BackgroundColor3=Color3.fromRGB(27,28,34),BorderSizePixel=0,PlaceholderText=ecfg.Name or "Input",Text="",TextColor3=Color3.fromRGB(235,235,240),TextSize=12,Font=Enum.Font.Gotham,ClearTextOnFocus=false,Size=UDim2.fromScale(1,1)},row);corner(box,6);local elem={Value=""};function elem:Set(v) elem.Value=tostring(v or "");box.Text=elem.Value;if ecfg.Callback then pcall(ecfg.Callback,elem.Value) end end;reg(ecfg.Flag or ecfg.Name,elem);return elem
            end
            function sec:AddKeybind(ecfg) return sec:AddButton({Name=(ecfg.Name or "Keybind")..": "..tostring(ecfg.Default or "None"),Callback=ecfg.Callback}) end
            function sec:AddDivider(dcfg) local l=label(sec.Root,dcfg and dcfg.Text or "",11,true);l.Size=UDim2.new(1,0,0,20);l.TextColor3=accent;return l end
            function sec:AddParagraph(ecfg) local l=label(sec.Root,(ecfg and (ecfg.Content or ecfg.Text or ecfg.Name)) or "",11,false);l.Size=UDim2.new(1,0,0,30);l.TextWrapped=true;return l end
            function sec:AddLabel(ecfg) return sec:AddParagraph(ecfg) end
            function sec:AddCenterTabbox(name) return nil end
            return sec
        end
        function tab:AddCenterTabbox(name)
            local host=mk("Frame",{BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y,Size=UDim2.new(1,0,0,0)},page)
            local bar=mk("Frame",{BackgroundColor3=Color3.fromRGB(14,15,18),BorderSizePixel=0,AutomaticSize=Enum.AutomaticSize.X,Size=UDim2.new(1,0,0,32)},host);corner(bar,6)
            local layout=mk("UIListLayout",{FillDirection=Enum.FillDirection.Horizontal,Padding=UDim.new(0,2)},bar)
            local pages=mk("Frame",{BackgroundTransparency=1,Position=UDim2.fromOffset(0,38),Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y},host)
            local box={}
            function box:AddTab(ccfg)
                local t={Name=ccfg.Name or "Subtab",ParentBox=box}
                local btn=mk("TextButton",{BackgroundTransparency=1,Text=t.Name,TextColor3=Color3.fromRGB(165,168,178),TextSize=11,Font=Enum.Font.GothamSemibold,Size=UDim2.fromOffset(math.max(70,#t.Name*7+22),32)},bar)
                local pg=mk("ScrollingFrame",{Visible=false,BackgroundTransparency=1,BorderSizePixel=0,Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,ScrollBarThickness=4,CanvasSize=UDim2.new()},pages);mk("UIListLayout",{Padding=UDim.new(0,8),SortOrder=Enum.SortOrder.LayoutOrder},pg);mk("UIPadding",{PaddingBottom=UDim.new(0,10)},pg)
                function t:AddSection(scfg) return ({AddSection=function() end}) end
                -- Reuse the same section implementation by making a lightweight page adapter.
                local adapter={}
                setmetatable(adapter,{__index=function(_,k)
                    if k=="AddSection" then return function(_,scfg)
                        local fake={AddSection=function() end}
                        -- create section using the parent tab's implementation, then move its root.
                        local sec=tab:AddSection(scfg); if sec.Root then sec.Root.Parent=pg end; return sec
                    end end
                    return tab[k]
                end})
                t.Page=pg
                btn.MouseButton1Click:Connect(function() for _,x in ipairs(pages:GetChildren()) do if x:IsA("ScrollingFrame") then x.Visible=false end end;pg.Visible=true end)
                return adapter
            end
            return box
        end
        b.MouseButton1Click:Connect(function()
            for _,x in pairs(self.Tabs) do
                if x.Page then x.Page.Visible=false end
                if x.Button then x.Button.BackgroundColor3=Color3.fromRGB(22,23,28); x.Button.TextColor3=Color3.fromRGB(175,178,188) end
            end
            page.Visible=true; b.BackgroundColor3=Color3.fromRGB(52,31,34); b.TextColor3=accent
            self._active=tab
            if mobile then setDrawer(false) end
        end)
        self.Tabs[tab.Name]=tab
        if not self._active then
            page.Visible = true
            b.BackgroundColor3 = Color3.fromRGB(52,31,34)
            b.TextColor3 = accent
            self._active = tab
        end
        return tab
    end
    function self:CreateHomeTab(hcfg) return self:AddTab({Name=hcfg.Name or "Dashboard"}) end
    function self:SetAccount(_) end
    function self:AttachMenuIcon(_) end
    function self:Notify(ncfg) print("[VD]",ncfg and (ncfg.Title or ncfg.Content or ncfg.Text) or "Notification") end
    return self
end

local MenuIcon = nil
local Window = StandaloneUI:Window({
    Title = "Violence District",
    Content = "Standalone UI • Clean Red",
    Size = UDim2.fromOffset(720, 500),
    Color = Color3.fromRGB(235,70,70),
})

Window:CreateHomeTab({Name="Dashboard"})

-- PC shortcut; mobile uses the visible close/open controls and touch navigation.
