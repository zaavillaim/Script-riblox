...
-- =====================================================
-- UI.LUA STYLE RENDERER
-- Uses the uploaded ui.lua layout: left navigation + right
-- independently scrolling content, compact mobile scaling.
-- =====================================================
local KYS_UI = (function()
local a = {}
...
local ModernV2 = { UserProfile = "Classic", Themes = {} }

local function wrapTab(raw, window)
    local tab = { _raw = raw, _window = window }

    function tab:AddSection(c)
        raw:section(tostring((c or {}).Name or (c or {}).Title or "Section"))
        return tab
    end
    function tab:AddLabel(c)
        return raw:label(tostring((c or {}).Text or (c or {}).Name or (c or {}).Content or ""))
    end
    function tab:AddParagraph(c)
        return raw:label(tostring((c or {}).Content or (c or {}).Description or (c or {}).Name or ""))
    end
    ...
    function tab:AddToggle(c)
        c = c or {}
        return raw:toggle(tostring(c.Name or c.Title or "Toggle"), c.Default == true, c.Callback)
    end
    function tab:AddSlider(c)
        c = c or {}
        return raw:slider(
            tostring(c.Name or c.Title or "Slider"),
            tonumber(c.Min) or 0,
            tonumber(c.Max) or 100,
            tonumber(c.Default) or tonumber(c.Min) or 0,
            c.Callback
        )
    end
    function tab:AddButton(c)
        c = c or {}
        return raw:button(tostring(c.Name or c.Title or "Button"), c.Callback)
    end
    function tab:AddTextInput(c)
        c = c or {}
        return raw:input(
            tostring(c.Name or c.Title or "Input"),
            c.Placeholder or c.PlaceholderText or "",
            c.Callback
        )
    end
    function tab:AddKeybind(c)
        c = c or {}
        -- ui.lua has no keybind widget; preserve a compatible state object.
        local o = { Value = c.Default, CurrentValue = c.Default }
    ...
    function tab:AddDropdown(c)
        ...
        local idx, value = 1, c.Default
        for i,v in ipairs(values) do if v == c.Default then idx = i break end end
        value = value or values[idx]
        local o = { Value = value, CurrentValue = value }
        raw:button(tostring(c.Name or c.Title or "Dropdown") .. "  [" .. tostring(value or "None") .. "]", function()
            if #values == 0 then return end
            idx = idx % #values + 1
            value = values[idx]
            o.Value, o.CurrentValue = value, value
    ...
    function tab:AddColorPicker(c)
        ...
        raw:button(tostring(c.Name or "Color") .. "  [tap]", function()
            idx = idx % #colors + 1
            o.Value, o.CurrentValue = colors[idx], colors[idx]
    ...
    function tab:AddCenterTabbox(name)
        -- ui.lua already provides the navigation model; feature sub-tabs
        -- are represented as a compact section inside the active page.
        local box = {}
        function box:AddTab(c)
            local sub = {}
            function sub:AddSection(x) return tab:AddSection(x) end
            function sub:AddLabel(x) return tab:AddLabel(x) end
            function sub:AddParagraph(x) return tab:AddParagraph(x) end
            function sub:AddDivider(x) return tab:AddDivider(x) end
            function sub:AddToggle(x) return tab:AddToggle(x) end
            function sub:AddSlider(x) return tab:AddSlider(x) end
            function sub:AddButton(x) return tab:AddButton(x) end
            function sub:AddTextInput(x) return tab:AddTextInput(x) end
            function sub:AddKeybind(x) return tab:AddKeybind(x) end
            function sub:AddDropdown(x) return tab:AddDropdown(x) end
            function sub:AddColorPicker(x) return tab:AddColorPicker(x) end
            return sub
        end
        return box
    end
...
function ModernV2:Window(c)
    c = c or {}
    local raw = KYS_UI.new(c.Title or "Violence District")
    local w = { _raw = raw, ConfigElements = {}, Tabs = {} }

    function w:AddTab(tc)
        tc = tc or {}
        local t = wrapTab(raw:tab(tostring(tc.Name or "Tab")), w)
        table.insert(self.Tabs, t)
        return t
    end
    function w:AttachMenuIcon() end
    function w:SetAccount() end
    function w:CreateHomeTab(hc)
        local t = self:AddTab({Name = (hc and hc.Name) or "Dashboard"})
        t:AddSection({Name = (hc and hc.Content) or "Dashboard"})
        return t
    end
    function w:Notify(n)
        if n and n.Content then
            warn("[Violence District] " .. tostring(n.Title or "Notice") .. ": " .. tostring(n.Content))
        end
    end
    return w
end

-- Instantiate immediately. ui.lua handles mobile scaling, touch, dragging,
-- sidebar navigation, and per-page scrolling.
