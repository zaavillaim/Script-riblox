...
local PerformanceTab = Window:AddTab("Performance", "rbxassetid://")

PerformanceTab:AddToggle("Anti Lag", {
    Text = "Anti Lag",
    Default = false,
    Callback = function(v)
        KYS_SetAntiLag(v)
...
PerformanceTab:AddButton("Ping Boost / Cleanup", function()
    KYS_PingBoost()
end)

local ConfigTab = Window:AddTab("Config", "rbxassetid://")
local KYS_ConfigName = "Default"

ConfigTab:AddTextInput("Config Name", {
    Default = "Default",
    Placeholder = "Config name",
    Callback = function(v)
        KYS_ConfigName = tostring(v ~= "" and v or "Default")
    end
})

ConfigTab:AddButton("Save Config", function()
    local ok, msg = KYS_SaveConfigV5(KYS_ConfigName)
    if Window.Notify then
        pcall(function() Window:Notify(ok and ("Saved: "..KYS_ConfigName) or ("Save failed: "..tostring(msg))) end)
    end
end)

ConfigTab:AddButton("Load Config", function()
    local ok, msg = KYS_LoadConfigV5(KYS_ConfigName)
    if Window.Notify then
        pcall(function() Window:Notify(ok and ("Loaded: "..KYS_ConfigName) or ("Load failed: "..tostring(msg))) end)
    end
end)

ConfigTab:AddButton("Delete Config", function()
    local ok, msg = KYS_DeleteConfigV5(KYS_ConfigName)
    if Window.Notify then
        pcall(function() Window:Notify(ok and ("Deleted: "..KYS_ConfigName) or ("Delete failed: "..tostring(msg))) end)
    end
end)

ConfigTab:AddButton("Refresh Config List", function()
    local list = KYS_ListConfigsV5()
    if Window.Notify then
        pcall(function()
            Window:Notify(#list > 0 and ("Configs: "..table.concat(list, ", ")) or "No configs found")
        end)
    end
end)

ConfigTab:AddButton("Set Auto Load", function()
    local ok = KYS_SetAutoLoadConfigV5(KYS_ConfigName)
    if Window.Notify then
        pcall(function() Window:Notify(ok and ("Auto Load: "..KYS_ConfigName) or "Auto Load unavailable") end)
    end
end)

ConfigTab:AddButton("Load Auto Config", function()
    KYS_AutoLoadConfigV5()
    if Window.Notify then pcall(function() Window:Notify("Auto config loaded") end) end
end)
...
-- =====================================================
-- EMBEDDED MODERN UI
-- Local UI implementation: no external ModernV2 dependency.
-- Layout: modern sidebar + content panel, compact/mobile friendly.
-- =====================================================
local ModernV2 = { UserProfile = "Embedded", Themes = {} }

local function KYS_parent()
    local p
...
local function KYS_corner(o, r)
    local c = Instance.new("UICorner")
    c.CornerRadius = UDim.new(0, r or 7)
    c.Parent = o
end

local function KYS_control(parent, window, cfg, kind)
    cfg = cfg or {}
    ...
    label.TextSize = 12
    label.TextColor3 = Color3.fromRGB(235,232,238)
    label.TextXAlignment = Enum.TextXAlignment.Left
    label.Text = tostring(cfg.Name or cfg.Title or "Control")
    label.Parent = row

    local obj = {Value = cfg.Default, CurrentValue = cfg.Default}
    function obj:Get() return self.Value end
    function obj:GetValue() return self.Value end
    function obj:Set(v)
        self.Value, self.CurrentValue = v, v
        if self._render then pcall(self._render) end
    ...
    if kind == "toggle" then
        ...
        thumb.BackgroundColor3 = Color3.fromRGB(245,243,247)
        thumb.BorderSizePixel = 0
        thumb.Parent = track
        KYS_corner(thumb,8)

        local function render()
            local on = obj.Value == true
            track.BackgroundColor3 = on and Color3.fromRGB(213,164,203) or Color3.fromRGB(62,58,68)
            thumb.Position = on and UDim2.new(1,-18,0,2) or UDim2.fromOffset(2,2)
        end
        obj._render = render
        render()
        b.Activated:Connect(function() obj:Set(not (obj.Value == true)) end)
    ...
    elseif kind == "button" then
        ...
        b.BackgroundColor3 = Color3.fromRGB(35,31,38)
        b.BorderSizePixel = 0
        b.TextColor3 = Color3.fromRGB(235,232,238)
        b.Font = Enum.Font.GothamMedium
        b.TextSize = 11
        b.Text = tostring(cfg.Text or cfg.Name or "Execute")
        b.Parent = row
        KYS_corner(b,6)
    ...
    elseif kind == "slider" then
        ...
        track.BackgroundColor3 = Color3.fromRGB(58,53,63)
        track.BorderSizePixel = 0
        track.Parent = b
        KYS_corner(track,3)

        local fill = Instance.new("Frame")
        fill.Size = UDim2.new(math.clamp((value-minv)/math.max(maxv-minv,1),0,1),0,1,0)
        fill.BackgroundColor3 = Color3.fromRGB(213,164,203)
        ...
        val.Size = UDim2.fromOffset(42,20)
        val.Font = Enum.Font.Gotham
        val.TextSize = 10
        val.TextColor3 = Color3.fromRGB(235,232,238)
        ...
        local function setFromX(x)
            ...
            v = minv + math.floor((v-minv)/inc + .5)*inc
            obj.Value, obj.CurrentValue = v,v
            val.Text = tostring(v)
            fill.Size = UDim2.new(math.clamp((v-minv)/math.max(maxv-minv,1),0,1),0,1,0)
            if cfg.Callback then pcall(cfg.Callback,v) end
        end
    ...
    elseif kind == "dropdown" then
        ...
        if multi then
            ...
            if type(def) == "table" then
                for _,v in ipairs(def) do selected[tostring(v)] = true end
            elseif def ~= nil then
                selected[tostring(def)] = true
            end
        else
            for i,v in ipairs(values) do
                if v == cfg.Default then idx=i break end
            end
        end
        ...
        b.AnchorPoint = Vector2.new(1,.5)
        b.Position = UDim2.new(1,0,.5,0)
        b.Size = UDim2.fromOffset(158,32)
        b.BackgroundColor3 = Color3.fromRGB(27,25,30)
        b.BorderSizePixel = 0
        b.TextColor3 = Color3.fromRGB(235,232,238)
        ...
        arrow.TextSize = 12
        arrow.TextColor3 = Color3.fromRGB(160,155,165)
        arrow.Parent = b
        ...
        popup.BackgroundColor3 = Color3.fromRGB(23,21,27)
        ...
        stroke.Color = Color3.fromRGB(58,53,63)
        stroke.Thickness = 1
        stroke.Parent = popup
        ...
        local function getText()
            if multi then
                ...
                for _,v in ipairs(values) do
                    if selected[tostring(v)] then table.insert(chosen, tostring(v)) end
                end
                return #chosen > 0 and table.concat(chosen, ", ") or "None"
            end
            return tostring(values[idx] or cfg.Default or "None")
        end
        ...
        local function emit()
            if cfg.Callback then
                if multi then
                    local result = {}
                    for _,v in ipairs(values) do
                        if selected[tostring(v)] then table.insert(result, v) end
                    end
                    pcall(cfg.Callback, result)
                else
                    pcall(cfg.Callback, values[idx] or cfg.Default)
                end
            end
        end
        ...
        local function render()
            ...
            for _,child in ipairs(list:GetChildren()) do
                if child:IsA("TextButton") then
                    local key = child:GetAttribute("KYSOption")
                    local isOn = multi and selected[tostring(key)]
                    child.BackgroundColor3 = isOn and Color3.fromRGB(70,57,70) or Color3.fromRGB(31,28,35)
                    child.TextColor3 = isOn and Color3.fromRGB(245,235,245) or Color3.fromRGB(210,206,214)
                    if multi then
                        child.Text = (isOn and "✓  " or "    ") .. tostring(key)
                    end
                end
            end
        end
        ...
        for _,v in ipairs(values) do
            ...
            opt.BackgroundColor3 = Color3.fromRGB(31,28,35)
            opt.BorderSizePixel = 0
            opt.AutoButtonColor = false
            opt.TextColor3 = Color3.fromRGB(210,206,214)
            ...
            opt.ZIndex = 102
            opt.Text = "    " .. tostring(v)
            opt:SetAttribute("KYSOption", tostring(v))
            opt.ZIndex = 52
            opt.Parent = list
            KYS_corner(opt,4)
        ...
        local function setOpen(state)
            ...
            if state then
                local h = math.min(190, math.max(38, #values * 30 + 8))
                popup.Size = UDim2.new(1,0,0,h)
                popup.ZIndex = 100
                popup.ClipsDescendants = true
                for _,c in ipairs(popup:GetDescendants()) do
                    if c:IsA("GuiObject") then c.ZIndex = math.max(c.ZIndex,51) end
                end
            end
        end

        render()
        b.Activated:Connect(function() setOpen(not open) end)

        function obj:Set(v)
            if multi then
                selected = {}
                if type(v) == "table" then
                    for _,x in ipairs(v) do selected[tostring(x)] = true end
                elseif v ~= nil then
        ...
        function obj:Get()
            ...
            return values[idx] or cfg.Default
        end
    ...
    elseif kind == "input" then
        ...
        b.PlaceholderText = tostring(cfg.Placeholder or "")
        b.Text = tostring(cfg.Default or "")
        b.TextColor3 = Color3.fromRGB(235,232,238)
        b.Font = Enum.Font.Gotham
        b.TextSize = 10
        b.Parent = row
        ...
        b.FocusLost:Connect(function(enter)
            obj.Value,obj.CurrentValue=b.Text,b.Text
            if cfg.Callback then pcall(cfg.Callback,b.Text,enter) end
        end)
        function obj:Set(v)
...
local function KYS_Tab(rawPage, window)
    local tab = {Page=rawPage,_window=window}
    function tab:AddSection(c)
        local x=Instance.new("TextLabel")
        x.BackgroundTransparency=1
        x.Size=UDim2.new(1,0,0,26)
        x.Font=Enum.Font.GothamSemibold
        x.TextSize=12
        x.TextColor3=Color3.fromRGB(213,164,203)
        x.TextXAlignment=Enum.TextXAlignment.Left
        x.Text=tostring((c or {}).Name or "SECTION"):upper()
        x.Parent=rawPage
        return tab
    end
    function tab:AddToggle(c) return KYS_control(rawPage,window,c,"toggle") end
    function tab:AddButton(c) return KYS_control(rawPage,window,c,"button") end
    function tab:AddSlider(c) return KYS_control(rawPage,window,c,"slider") end
    function tab:AddDropdown(c) return KYS_control(rawPage,window,c,"dropdown") end
    function tab:AddTextInput(c) return KYS_control(rawPage,window,c,"input") end
    function tab:AddLabel(c) return KYS_control(rawPage,window,c,"label") end
    function tab:AddParagraph(c)
        c=c or {}; c.Name=c.Name or "Info"; c.Content=c.Content or c.Description or ""
        local x=Instance.new("TextLabel")
        x.BackgroundTransparency=1; x.Size=UDim2.new(1,0,0,40)
        x.Font=Enum.Font.Gotham; x.TextSize=11; x.TextColor3=Color3.fromRGB(190,187,196)
        x.TextWrapped=true; x.TextXAlignment=Enum.TextXAlignment.Left
        x.Text=c.Content; x.Parent=rawPage; return x
    end
    function tab:AddDivider()
        local x=Instance.new("Frame"); x.Size=UDim2.new(1,0,0,1)
        x.BorderSizePixel=0; x.BackgroundColor3=Color3.fromRGB(48,46,55); x.Parent=rawPage
        return x
    end
    function tab:AddColorPicker(c)
        c=c or {}
        local colors={
            Color3.fromRGB(213,164,203),
        ...
        local idx=1
        local initial=c.Default or colors[1]
        for i,v in ipairs(colors) do
            if v == initial then idx=i break end
        end
        local o={Value=initial,CurrentValue=initial}
        local row=Instance.new("Frame")
        row.BackgroundTransparency=1
        row.Size=UDim2.new(1,0,0,42)
        row.Parent=rawPage
        ...
        label.TextColor3=Color3.fromRGB(235,232,238)
        label.TextXAlignment=Enum.TextXAlignment.Left
        label.Text=tostring(c.Name or "ESP Color")
        label.Parent=row

        local swatch=Instance.new("TextButton")
        swatch.AnchorPoint=Vector2.new(1,.5)
        swatch.Position=UDim2.new(1,0,.5,0)
        swatch.Size=UDim2.fromOffset(48,28)
        swatch.Text=""
        ...
        local stroke=Instance.new("UIStroke")
        stroke.Color=Color3.fromRGB(80,76,88)
        stroke.Thickness=1
        stroke.Parent=swatch

        local function apply(v,callback)
            o.Value,o.CurrentValue=v,v
            swatch.BackgroundColor3=v
            if callback and c.Callback then pcall(c.Callback,v) end
        end
        swatch.Activated:Connect(function()
            idx=idx%#colors+1
            apply(colors[idx],true)
        end)
    ...
    function tab:AddKeybind(c)
        local o={Value=(c or {}).Default,CurrentValue=(c or {}).Default}
        function o:Set(v) self.Value,self.CurrentValue=v,v end
        function o:Get() return self.Value end
        return o
    end
    function tab:AddCenterTabbox()
        local box={}
        function box:AddTab()
            return tab
        end
...
function ModernV2:CreateMenuIcon(c)
    local b=Instance.new("TextButton")
    b.Name="KYS_Modern_MenuIcon"
    b.Size=UDim2.fromOffset(44,44)
    b.Position=UDim2.new(0,12,.5,-22)
    b.BackgroundColor3=Color3.fromRGB(24,21,27)
    b.Text="☰"; b.TextColor3=Color3.fromRGB(213,164,203)
    b.TextSize=19; b.Font=Enum.Font.GothamBold
    b.AutoButtonColor=false; b.ZIndex=50
    KYS_corner(b,12)
    b.Parent=self._gui
    return b
end

function ModernV2:Window(c)
    c=c or {}
    local w={ConfigElements={},Tabs={}}
    local gui=Instance.new("ScreenGui")
    gui.Name="KYS_Modern_Embedded"
    gui.ResetOnSpawn=false
    gui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling
    gui.Parent=KYS_parent()
    self._gui=gui; w._gui=gui

    local shadow=Instance.new("Frame")
    shadow.Size=UDim2.fromOffset(484,324)
    shadow.Position=UDim2.new(.5,-242,.5,-162)
    shadow.BackgroundColor3=Color3.new(0,0,0)
    shadow.BackgroundTransparency=.55
    shadow.BorderSizePixel=0; shadow.Parent=gui; KYS_corner(shadow,12)

    local main=Instance.new("Frame")
    main.Size=UDim2.fromOffset(470,310)
    main.Position=UDim2.new(.5,-235,.5,-155)
    main.BackgroundColor3=Color3.fromRGB(17,17,21)
    main.BorderSizePixel=0; main.Active=true; main.Parent=gui; KYS_corner(main,10)
    w.Main=main

    local scale=Instance.new("UIScale"); scale.Parent=main
    local scale2=Instance.new("UIScale"); scale2.Parent=shadow
    local function resize()
        local cam=workspace.CurrentCamera
        if not cam then return end
        local v=cam.ViewportSize
        local z=math.min(1,(v.X-24)/470,(v.Y-24)/310)
        scale.Scale=z; scale2.Scale=z
    end
    resize()
    if workspace.CurrentCamera then workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(resize) end

    local title=Instance.new("TextLabel")
    title.Size=UDim2.new(1,-18,0,28); title.Position=UDim2.fromOffset(9,0)
    title.BackgroundTransparency=1; title.Text=tostring(c.Title or "Violence District") .. (isMobile and "  •  Touch UI" or "")
    title.TextColor3=Color3.fromRGB(235,232,238); title.Font=Enum.Font.GothamMedium
    title.TextSize=12; title.TextXAlignment=Enum.TextXAlignment.Left; title.Parent=main

    local side=Instance.new("Frame")
    side.Position=UDim2.fromOffset(0,28); side.Size=UDim2.new(0,90,1,-28)
    side.BackgroundColor3=Color3.fromRGB(14,14,18); side.BorderSizePixel=0; side.Parent=main

    local nav=Instance.new("ScrollingFrame")
    nav.Position=UDim2.fromOffset(5,7); nav.Size=UDim2.new(1,-10,1,-14)
    nav.BackgroundTransparency=1; nav.BorderSizePixel=0; nav.ScrollBarThickness=0
    nav.AutomaticCanvasSize=Enum.AutomaticSize.Y; nav.Parent=side
    local nl=Instance.new("UIListLayout"); nl.Padding=UDim.new(0,2); nl.Parent=nav

    local content=Instance.new("Frame")
    content.Position=UDim2.fromOffset(90,28); content.Size=UDim2.new(1,-90,1,-28)
    content.BackgroundColor3=Color3.fromRGB(17,17,21); content.BorderSizePixel=0; content.Parent=main

    function w:AddTab(tc)
        tc=tc or {}
        local btn=Instance.new("TextButton")
        btn.Size=UDim2.new(1,0,0,30); btn.BackgroundTransparency=1
        btn.Text=tostring(tc.Name or "Tab"):upper()
        btn.TextColor3=Color3.fromRGB(139,134,149)
        btn.Font=Enum.Font.GothamMedium; btn.TextSize=10
        btn.TextXAlignment=Enum.TextXAlignment.Left; btn.AutoButtonColor=false; btn.Parent=nav

        local marker=Instance.new("Frame")
        marker.Size=UDim2.fromOffset(2,16); marker.Position=UDim2.new(0,0,.5,-8)
        marker.BackgroundColor3=Color3.fromRGB(213,164,203); marker.BorderSizePixel=0
        marker.Visible=false; marker.Parent=btn; KYS_corner(marker,2)

        local page=Instance.new("ScrollingFrame")
        page.Size=UDim2.new(1,0,1,0); page.BackgroundTransparency=1
        page.BorderSizePixel=0; page.ScrollBarThickness=2
        page.ScrollBarImageColor3=Color3.fromRGB(213,164,203)
        page.AutomaticCanvasSize=Enum.AutomaticSize.Y; page.CanvasSize=UDim2.new()
        page.Visible=false; page.Parent=content
        local pad=Instance.new("UIPadding"); pad.PaddingTop=UDim.new(0,12)
        pad.PaddingBottom=UDim.new(0,12); pad.PaddingLeft=UDim.new(0,14); pad.PaddingRight=UDim.new(0,14); pad.Parent=page
        local lay=Instance.new("UIListLayout"); lay.Padding=UDim.new(0,6); lay.Parent=page

        local t=KYS_Tab(page,w); t.Button=btn; t.Marker=marker
        function t:Show()
            for _,x in ipairs(w.Tabs) do
                x.Page.Visible=false; x.Button.TextColor3=Color3.fromRGB(139,134,149)
                x.Marker.Visible=false
            end
            page.Visible=true; btn.TextColor3=Color3.fromRGB(235,232,238); marker.Visible=true
        end
        table.insert(w.Tabs,t)
        btn.Activated:Connect(function() t:Show() end)
        if #w.Tabs==1 then t:Show() end
        return t
    end

    function w:AttachMenuIcon(icon)
    ...
    function w:SetAccount() end
    function w:CreateHomeTab(hc)
        local t=self:AddTab({Name=(hc or {}).Name or "Dashboard"})
        t:AddSection({Name=(hc or {}).Content or "Dashboard"})
        return t
    end
    function w:Notify(n)
        if n and n.Content then warn("[KYSHub] "..tostring(n.Title or "Notice")..": "..tostring(n.Content)) end
    end
    return w
end
...
if Window then
    ...
    -- Role menus are deliberately separated like the original layout:
    -- Survivor features live under a dedicated Survivor top-level tab,
    -- Killer features live under a dedicated Killer top-level tab.
    MainFeatureTabs = addCenterFeatureTabbox(Tabs.Survivor, "Survivor Features", {
    ...
    MainKillerFeatureTabs = addCenterFeatureTabbox(Tabs.Killer, "Killer Features", {
