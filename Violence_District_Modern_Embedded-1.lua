...
local function KYS_control(parent, window, cfg, kind)
    ...
    if kind == "toggle" then
        ...
        local function render()
            ...
            b.BackgroundColor3 = on and Color3.fromRGB(213,164,203) or Color3.fromRGB(62,58,68)
        end
        obj._render = render
        render()
    ...
    elseif kind == "dropdown" then
        ...
        local idx = 1
        for i,v in ipairs(values) do if v == cfg.Default then idx=i break end end
        local b = Instance.new("TextButton")
        b.AnchorPoint = Vector2.new(1,.5)
        b.Position = UDim2.new(1,0,.5,0)
        ...
        KYS_corner(b,5)
        local function render()
            obj.Value = values[idx] or cfg.Default
            obj.CurrentValue = obj.Value
            b.Text = "  " .. tostring(obj.Value or "None")
        end
        render()
        b.Activated:Connect(function()
            if #values > 0 then
        ...
        function obj:Set(v)
            for i,x in ipairs(values) do if x==v then idx=i break end end
            render()
            if cfg.Callback then pcall(cfg.Callback,obj.Value) end
        end
