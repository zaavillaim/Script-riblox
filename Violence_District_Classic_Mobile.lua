...
-- =====================================================
-- CLASSIC MOBILE UI
-- Embedded UI: no ModernV2/remote UI dependency.
-- Opens immediately on execute and supports touch + mouse.
-- =====================================================
local ModernV2 = {}
ModernV2.UserProfile = "Classic"
ModernV2.Themes = {}

local function KYS_MakeControl(parent, window, cfg, kind)
    cfg = cfg or {}
    local holder = Instance.new("Frame")
    holder.BackgroundTransparency = 1
    holder.Size = UDim2.new(1, -10, 0, kind == "paragraph" and 58 or 42)
    holder.Parent = parent

    local label = Instance.new("TextLabel")
    label.BackgroundTransparency = 1
    label.Position = UDim2.fromOffset(8, 3)
    label.Size = UDim2.new(0.58, -8, 1, -6)
    ...
    local state = {
        ...
        Config = { Value = cfg.Default, Default = cfg.Default },
        Default = cfg.Default,
        Name = cfg.Name,
    }

    local function invoke(v)
        state.Value = v
        state.CurrentValue = v
        state.Config.Value = v
    ...
    function state:Set(v)
        invoke(v)
        if state._render then pcall(state._render) end
    end
    function state:Get() return state.Value end
    function state:GetValue() return state.Value end
    ...
    if kind == "toggle" then
        ...
        local function render()
            local on = state.Value == true
            button.Text = on and "ON" or "OFF"
            button.BackgroundColor3 = on and Color3.fromRGB(235,70,70) or Color3.fromRGB(55,57,64)
        end
        state._render = render
        render()
        button.Activated:Connect(function() state:Set(not (state.Value == true)) end)

    elseif kind == "button" then
        label.Size = UDim2.new(0.62, -8, 1, -6)
        local button = Instance.new("TextButton")
        button.Position = UDim2.new(1, -128, 0.5, -14)
        button.Size = UDim2.fromOffset(118, 28)
        button.BackgroundColor3 = Color3.fromRGB(235,70,70)
        button.Text = "EXECUTE"
        button.Font = Enum.Font.GothamBold
        button.TextSize = 11
        button.TextColor3 = Color3.new(1,1,1)
    ...
    elseif kind == "slider" then
        ...
        box.BackgroundColor3 = Color3.fromRGB(42,44,51)
        box.ClearTextOnFocus = false
        box.Font = Enum.Font.Gotham
        box.TextSize = 12
        box.TextColor3 = Color3.new(1,1,1)
        box.Text = tostring(cfg.Default or cfg.Min or 0)
        box.Parent = holder
        local function clamp(v)
            v = tonumber(v) or tonumber(cfg.Min) or 0
            if cfg.Min ~= nil then v = math.max(v, cfg.Min) end
            if cfg.Max ~= nil then v = math.min(v, cfg.Max) end
            local inc = tonumber(cfg.Increment)
            if inc and inc > 0 then
                local base = tonumber(cfg.Min) or 0
                v = base + math.floor((v-base)/inc + 0.5)*inc
            end
            return v
        end
        state.Value = clamp(cfg.Default or cfg.Min or 0)
        state.Config.Value = state.Value
        box.Text = tostring(state.Value)
        function state:Set(v)
            v = clamp(v)
            state.Value, state.CurrentValue, state.Config.Value = v,v,v
            box.Text = tostring(v)
    ...
    elseif kind == "dropdown" then
        ...
        local button = Instance.new("TextButton")
        button.Position = UDim2.new(1, -188, 0.5, -14)
        button.Size = UDim2.fromOffset(178, 28)
        button.BackgroundColor3 = Color3.fromRGB(42,44,51)
        ...
        local function display()
            if multi then
                ...
                button.Text = "  " .. (#out > 0 and table.concat(out, ", ") or "  None")
            else
                button.Text = "  " .. tostring(values[index] or cfg.Default or "None")
            end
        end
        display()
        ...
        function state:Set(v)
            ...
            state.CurrentValue, state.Config.Value = state.Value, state.Value
            display()
            if cfg.Callback then pcall(cfg.Callback, state.Value) end
        end
        function state:SetValues(newValues)
            values = newValues or {}
        ...
        button.Activated:Connect(function()
            ...
            if multi then
                ...
                local out = {}
                for _,x in ipairs(values) do if selected[x] then table.insert(out,x) end end
                state.Value = out
                state.CurrentValue, state.Config.Value = out,out
                display()
    ...
    elseif kind == "color" then
        local colors = {
            Color3.fromRGB(235,70,70), Color3.fromRGB(70,170,255),
            Color3.fromRGB(80,220,130), Color3.fromRGB(255,210,70),
            Color3.fromRGB(190,90,255), Color3.fromRGB(255,255,255)
        }
        local idx = 1
        local button = Instance.new("TextButton")
        ...
        button.TextColor3 = Color3.new(1,1,1)
        button.Parent = holder
        state.Value = cfg.Default or colors[1]
        local function render()
            button.BackgroundColor3 = state.Value
            button.Text = "COLOR"
        end
        render()
        button.Activated:Connect(function()
            idx = idx % #colors + 1
            state:Set(colors[idx])
        end)

    elseif kind == "keybind" then
        local button = Instance.new("TextButton")
        button.Position = UDim2.new(1, -128, 0.5, -13)
        button.Size = UDim2.fromOffset(118, 26)
        button.BackgroundColor3 = Color3.fromRGB(42,44,51)
        button.Font = Enum.Font.Gotham
        button.TextSize = 11
        button.TextColor3 = Color3.new(1,1,1)
        button.Text = tostring(cfg.Default or "None")
        button.Parent = holder
        state.Value = cfg.Default
        button.Activated:Connect(function()
    ...
    elseif kind == "textinput" then
        local box = Instance.new("TextBox")
        box.Position = UDim2.new(1, -188, 0.5, -14)
        box.Size = UDim2.fromOffset(178, 28)
        box.BackgroundColor3 = Color3.fromRGB(42,44,51)
        box.ClearTextOnFocus = false
        box.Font = Enum.Font.Gotham
        box.TextSize = 11
        box.TextColor3 = Color3.new(1,1,1)
        box.PlaceholderText = tostring(cfg.Placeholder or "Enter text")
    ...
    elseif kind == "paragraph" or kind == "label" then
        holder.Size = UDim2.new(1, -10, 0, kind == "paragraph" and 62 or 36)
        label.Size = UDim2.new(1, -16, 1, -6)
        label.TextWrapped = true
        label.TextYAlignment = Enum.TextYAlignment.Top
        label.Text = tostring(cfg.Content or cfg.Description or cfg.Text or cfg.Name or "")
    end

    if cfg.Flag then
        window.ConfigElements[cfg.Flag] = state
    end
    return state
end

local function KYS_MakeContainer(parent)
    local container = {}
    container.Frame = parent

    function container:AddSection(cfg)
        local h = Instance.new("TextLabel")
        h.BackgroundTransparency = 1
        h.Size = UDim2.new(1, -16, 0, 30)
        h.Font = Enum.Font.GothamBold
        h.TextSize = 14
        h.TextColor3 = Color3.fromRGB(235,70,70)
        h.TextXAlignment = Enum.TextXAlignment.Left
        h.Text = tostring((cfg and cfg.Name) or "Section")
        h.Parent = parent
        return KYS_MakeContainer(parent)
    end
    function container:AddDivider(cfg)
        local d = Instance.new("Frame")
        d.BorderSizePixel = 0
        d.BackgroundColor3 = Color3.fromRGB(55,57,64)
        d.Size = UDim2.new(1, -16, 0, 1)
        d.Parent = parent
        return d
    end
    function container:AddToggle(cfg) return KYS_MakeControl(parent, container._window, cfg, "toggle") end
    function container:AddButton(cfg) return KYS_MakeControl(parent, container._window, cfg, "button") end
    function container:AddSlider(cfg) return KYS_MakeControl(parent, container._window, cfg, "slider") end
    function container:AddDropdown(cfg) return KYS_MakeControl(parent, container._window, cfg, "dropdown") end
    function container:AddColorPicker(cfg) return KYS_MakeControl(parent, container._window, cfg, "color") end
    function container:AddKeybind(cfg) return KYS_MakeControl(parent, container._window, cfg, "keybind") end
    function container:AddTextInput(cfg) return KYS_MakeControl(parent, container._window, cfg, "textinput") end
    function container:AddParagraph(cfg) return KYS_MakeControl(parent, container._window, cfg, "paragraph") end
    function container:AddLabel(cfg) return KYS_MakeControl(parent, container._window, cfg, "label") end

    function container:AddCenterTabbox(name)
        local holder = Instance.new("Frame")
        holder.BackgroundTransparency = 1
        holder.Size = UDim2.new(1, -8, 0, 420)
        holder.Parent = parent
        ...
        title.TextSize = 13
        title.TextColor3 = Color3.fromRGB(235,70,70)
        title.TextXAlignment = Enum.TextXAlignment.Left
        title.Text = tostring(name or "Features")
        title.Parent = holder
        ...
        local pages = {}
        local box = { _window = container._window }

        function box:AddTab(tabCfg)
            local page = Instance.new("ScrollingFrame")
            page.BackgroundTransparency = 1
            page.Position = UDim2.fromOffset(0,61)
            page.Size = UDim2.new(1,0,1,-61)
            ...
            button.BackgroundColor3 = (#pages == 0) and Color3.fromRGB(235,70,70) or Color3.fromRGB(42,44,51)
            button.TextColor3 = Color3.fromRGB(235,235,240)
            button.Font = Enum.Font.GothamBold
            button.TextSize = 10
            button.Text = tostring(tabCfg.Name or "Feature")
            button.Parent = bar

            local tab = KYS_MakeContainer(page)
            tab._window = container._window
            tab.Page = page
            tab.Button = button
            ...
            button.Activated:Connect(function()
                for _, item in ipairs(pages) do
                    ...
                    item.Button.BackgroundColor3 = Color3.fromRGB(42,44,51)
                end
                page.Visible = true
                button.BackgroundColor3 = Color3.fromRGB(235,70,70)
            end)
...
function ModernV2:AddTheme(theme) self.Theme = theme end

function ModernV2:CreateMenuIcon(cfg)
    local b = Instance.new("TextButton")
    b.Name = "KYS_ClassicMenuIcon"
    b.Size = UDim2.fromOffset(48,48)
    b.Position = UDim2.new(0, 14, 0.5, -24)
    b.BackgroundColor3 = Color3.fromRGB(18,19,23)
    b.Text = "☰"
    b.TextSize = 22
    b.TextColor3 = Color3.fromRGB(235,70,70)
    b.ZIndex = 100
    b.Parent = self._gui
    return b
end
...
function ModernV2:Window(cfg)
    ...
    window.Tabs = {}
    window._gui = Instance.new("ScreenGui")
    window._gui.Name = "KYS_ClassicUI"
    window._gui.ResetOnSpawn = false
    window._gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    ...
    window._gui.Parent = parentGui
    self._gui = window._gui

    local main = Instance.new("Frame")
    main.Name = "Main"
    main.Size = isMobile and UDim2.new(1,-18,1,-110) or UDim2.fromOffset(720,500)
    main.Position = isMobile and UDim2.new(0,9,0,9) or UDim2.new(0.5,-360,0.5,-250)
    main.BackgroundColor3 = Color3.fromRGB(20,21,26)
    main.BorderSizePixel = 0
    main.Active = true
    main.Parent = window._gui
    ...
    title.TextXAlignment = Enum.TextXAlignment.Left
    title.Text = tostring(cfg.Title or "Violence District")
    title.Parent = main
    ...
    subtitle.TextColor3 = Color3.fromRGB(160,163,170)
    subtitle.TextXAlignment = Enum.TextXAlignment.Left
    subtitle.Text = tostring(cfg.Content or "Classic Mobile UI")
    subtitle.Parent = main

    local tabsBar = Instance.new("ScrollingFrame")
    tabsBar.BackgroundTransparency = 1
    tabsBar.Position = UDim2.fromOffset(8,64)
    ...
    content.Position = UDim2.fromOffset(8,106)
    content.Size = UDim2.new(1,-16,1,-114)
    content.Parent = main

    function window:AddTab(tabCfg)
        local page = Instance.new("ScrollingFrame")
        page.Name = tostring(tabCfg.Name or "Tab")
        page.BackgroundTransparency = 1
        ...
        tabButton.Size = UDim2.fromOffset(105,32)
        tabButton.BackgroundColor3 = Color3.fromRGB(42,44,51)
        tabButton.TextColor3 = Color3.fromRGB(235,235,240)
        tabButton.Font = Enum.Font.GothamBold
        tabButton.TextSize = 11
        tabButton.Text = tostring(tabCfg.Name or "Tab")
        ...
        tabButton.Activated:Connect(function()
            for _,t in ipairs(self.Tabs) do
                ...
                t.Button.BackgroundColor3 = Color3.fromRGB(42,44,51)
            end
            page.Visible = true
            tabButton.BackgroundColor3 = Color3.fromRGB(235,70,70)
        end)

        table.insert(self.Tabs, tab)
        if #self.Tabs == 1 then tabButton.BackgroundColor3 = Color3.fromRGB(235,70,70) end
        return tab
    end

    function window:AttachMenuIcon(icon)
        if not icon then return end
        icon.Parent = window._gui
        icon.Activated:Connect(function() main.Visible = not main.Visible end)
    end
    function window:SetAccount() end
    function window:CreateHomeTab(homeCfg)
        local home = self:AddTab({Name = homeCfg.Name or "Dashboard"})
        home:AddSection({Name = homeCfg.Content or "Dashboard"})
        home:AddParagraph({Name = "Info", Content = "Classic UI loaded successfully. Features are available in the tabs above."})
    end
    function window:Notify(cfg)
        local msg = Instance.new("TextLabel")
        msg.Size = UDim2.new(1,-30,0,44)
        msg.Position = UDim2.new(0,15,1,-55)
        msg.BackgroundColor3 = Color3.fromRGB(35,36,42)
        msg.TextColor3 = Color3.new(1,1,1)
        msg.Font = Enum.Font.Gotham
        msg.TextSize = 12
        msg.TextWrapped = true
        msg.Text = tostring(cfg.Title or "Notice") .. ": " .. tostring(cfg.Content or "")
        msg.ZIndex = 200
        msg.Parent = window._gui
        task.delay(cfg.Duration or 2, function() pcall(function() msg:Destroy() end) end)
    end
    return window
end
