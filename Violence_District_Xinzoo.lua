local function __Xinzoo_Init_Main__()
...
if not ModernV2 then
    ...
    local loaderOk, loaderResult = pcall(function()
        local source = game:HttpGet("https://raw.githubusercontent.com/Kys-lol/XinzooNewUI/refs/heads/main/ModernLua.txt")
    ...
    if loaderOk then
        ...
        warn("[Xinzoo] Vercel mirror failed, trying GitHub fallback:", loaderResult)
        -- Fallback: GitHub raw (may be rate-limited)
        local fallbackOk, fallbackResult = pcall(function()
            local source = game:HttpGet("https://raw.githubusercontent.com/Kys-lol/XinzooNewUI/refs/heads/main/MainV2.lua")
            local fn, compileErr = loadstring(source)
            if not fn then error(compileErr) end
            return fn()
        end)
        if fallbackOk then
            ...
            warn("[Xinzoo] Failed to load ModernV2 from all sources:", fallbackResult)
        end
    end
end
...
local UIConfigPath = "Xinzoo_UI_Theme.json"
...
if ModernV2 then
    ...
    Window = ModernV2:Window({
        Title = "Violence District | Xinzoo",
        ...
        Config = {
            ConfigFolder = "XinzooViolenceDistrict",
            AutoSaveFile = "KYS_VD",
            AutoSave = false,
            AutoLoad = false,
            Overwrite = true,
...
local ConfigFolderName = "Xinzoo_VD"
local HttpService = game:GetService("HttpService")
...
-- CHARACTER REFS
-- =====================================================
-- Character, Humanoid, and Root are declared at the top of __Xinzoo_Init_Main__
...
do
    ...
    local function KYS_GetESPFolder()
        ...
        local old = parent:FindFirstChild("Xinzoo_VisualESP") or parent:FindFirstChild("ZiaanHub_ESP")
        if old then old:Destroy() end

        local folder = Instance.new("Folder")
        folder.Name = "Xinzoo_VisualESP"
...
pcall(function()
    VD_Notify("Xinzoo crack", "Violence District v1.5.7 Loaded Successfully!", 5)
end)
...
function setupSpearAimbotBtn()
    ...
    print("[Xinzoo crack] Spear Aimbot Button created successfully")
end
...
local function KillPremium()
    ...
    print("[Xinzoo] Premium-ограничения отключены.")
end
