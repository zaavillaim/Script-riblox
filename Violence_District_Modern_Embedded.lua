...
local function KYS_Tab(rawPage, window)
    ...
    function tab:AddColorPicker(c)
        c=c or {}
        c.Name=(c.Name or "Color").." (cycle)"
        local colors={Color3.fromRGB(213,164,203),Color3.fromRGB(70,170,255),Color3.fromRGB(80,220,130),Color3.fromRGB(255,210,70)}
        local idx=1
        local o={Value=c.Default or colors[1],CurrentValue=c.Default or colors[1]}
        KYS_control(rawPage,window,{Name=c.Name,Text="COLOR",Callback=function()
            idx=idx%#colors+1; o.Value,o.CurrentValue=colors[idx],colors[idx]
            if c.Callback then pcall(c.Callback,o.Value) end
        end},"button")
        function o:Set(v) self.Value,self.CurrentValue=v,v;if c.Callback then pcall(c.Callback,v) end end
...
function ModernV2:Window(c)
    ...
    title.BackgroundTransparency=1; title.Text=tostring(c.Title or "Violence District")
